                                ...
                                int       command)
{
   ...
   LOGE("[PROCESSMGR] pointed_win=%#x Send kill command to the ResourceD! PID=%d\n", bd->client.win, bd->client.netwm.pid);
...
_e_mod_processmgr_anr_ping_confirm_handler(void *data)
{
   ...
   if (!pointed_win) return ECORE_CALLBACK_CANCEL;
...
static void
_e_mod_processmgr_send_mDNIe_action(int pid, E_ProcessMgr_mDNIe_Action act)
{
   E_ProcessMgr *pm = NULL;
   DBusConnection *conn;
   DBusMessage* msg;
   int param_pid;
   int param_act;
   static int latest_fg_pid = -1;

   if ( latest_fg_pid == pid )
       return;

   LOGW("[PROCESSMGR] =====================> Broadcast mDNIeStatus : PID=%d", pid);
   latest_fg_pid = pid;
...
static Eina_Bool
_e_mod_processmgr_send_mDNIe_to_topmost_visible_window(E_Border *temp_bd)
{
   E_Border_List *bl;
   E_Border *bd = NULL;

   if (!temp_bd) return EINA_FALSE;
   if (!temp_bd->zone) return EINA_FALSE;
   if (!temp_bd->zone->container) return EINA_FALSE;

   bl = e_container_border_list_last(temp_bd->zone->container);
   while ((bd = e_container_border_list_prev(bl)))
     {
        // check visual region!!
        //LOGW("[PROCESSMGR] vis=%d x,y=%d,%d  zone->x,y,w,h=%d,%d,%d,%d", bd->visible, bd->x, bd->y, bd->zone->x, bd->zone->y, bd->zone->w, bd->zone->h);
        if (bd->visible && E_INSIDE(bd->x, bd->y, bd->zone->x, bd->zone->y, bd->zone->w, bd->zone->h) )
          {
             _e_mod_processmgr_send_mDNIe_action(bd->client.netwm.pid, mDNIe_FOREGROUND);
             break;
          }
     }
                         ...
                         void      *event)
{
   ...
   _e_mod_processmgr_send_mDNIe_to_topmost_visible_window(ev->border);
...
static Eina_Bool
_e_mod_processmgr_bd_show(void *data __UNUSED__,
                         int type   __UNUSED__,
                         void      *event)
{
...
static Eina_Bool
_e_mod_processmgr_bd_hide(void *data __UNUSED__,
                         int type   __UNUSED__,
                         void      *event)
{
...
static Eina_Bool
_e_mod_processmgr_bd_stack(void *data __UNUSED__,
                         int type   __UNUSED__,
                         void      *event)
{
...
static Eina_Bool
_e_mod_processmgr_bd_move(void *data __UNUSED__,
                         int type   __UNUSED__,
                         void      *event)
{
                               ...
                               void      *event)
{
   ...
   //LOGW("[PROCESSMGR] bd_uniconify");
   //LOGW("[PROCESSMGR] ev->border->win=%#x, bd->client.win=%#x", ev->border->win, ev->border->client.win);
   _e_mod_processmgr_send_mDNIe_to_topmost_visible_window(ev->border);
...
_e_mod_processmgr_send_pid_action(int pid, E_ProcessMgr_Action act)
{
...
#if 0
   if ( (act == PROCESS_FOREGROUND) || (act == PROCESS_LAUNCH) || (act == PROCESS_RESUME) )
      _e_mod_processmgr_send_mDNIe_action(pid, mDNIe_FOREGROUND);
#endif
   ...
   SLOGI("[PROCESSMGR] =====================> Send PID=%d, Action=%d", pid, act);
   // cleanup
   dbus_message_unref(msg);
}
...
e_mod_processmgr_init(void)
{
   ...
   handlers = eina_list_append(handlers, ecore_event_handler_add(E_EVENT_BORDER_SHOW,                  _e_mod_processmgr_bd_show,                NULL));
   handlers = eina_list_append(handlers, ecore_event_handler_add(E_EVENT_BORDER_HIDE,                  _e_mod_processmgr_bd_hide,                NULL));
   handlers = eina_list_append(handlers, ecore_event_handler_add(E_EVENT_BORDER_STACK,                  _e_mod_processmgr_bd_stack,                NULL));
   handlers = eina_list_append(handlers, ecore_event_handler_add(E_EVENT_BORDER_MOVE,                  _e_mod_processmgr_bd_move,                NULL));
