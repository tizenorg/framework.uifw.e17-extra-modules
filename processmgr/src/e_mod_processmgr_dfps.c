...
EINTERN int _e_mod_processmgr_dfps_init(void)
{
   ...
   /* register dbus signal handler for fps control with overheating. */
...
static void _dfps_action(dfps_app_info* app_info, int pid, dfps_app_state state)
{
   int dfps_rate = 0;
   ...
   switch (state) {
      ...
      case DFPS_PAUSE_STATE:
         if (_dfps_app_pid_exists(pid)) {
              dfps_rate = 60; // default
              _dfps_set_dfps_rate(pid, dfps_rate);
         }
         break;
      case DFPS_TERMINATED_STATE:
         if (_dfps_app_pid_exists(pid)) {
              _dfps_app_clear_pid(pid);
              dfps_rate = 0;
              _dfps_set_dfps_rate(pid, dfps_rate);
         }
      default:
   ...
   if ( pid )
     {
        ...
        if (app_info.package_name && app_info.app_name)
          {
             _dfps_action(&app_info, pid, state);
          }
