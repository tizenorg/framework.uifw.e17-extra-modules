             ...
             if(ft_bd->pinon)
               {
                  if(ft_bd->icon.win)
                    {
                       e_border_act_close_begin(ft_bd->icon.win->border);
                    }
               }
...
_policy_floating_pinwin_create(E_Illume_Floating_Border *ft_bd)
{
   ...
   EINA_SAFETY_ON_NULL_GOTO(edje_object_file_set(layout, buf, "mw_minimize_mode"), end);
   ...
   if(!swallow_obj)
     {
        ...
        snprintf(buf, sizeof(buf), "%s/e-module-illume2-tizen.edj", _e_illume_mod_dir);
        EINA_SAFETY_ON_NULL_GOTO(edje_object_file_set(swallow_obj, buf, "default_icon"), end);
     }
