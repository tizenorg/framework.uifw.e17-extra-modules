...
static void                  _effect_below_floating_wins_set(E_Comp_Win *cw, Eina_Bool recreate);
...
EINTERN void
e_mod_comp_effect_object_win_set(E_Comp_Win *cw, const char *emission)
{
   _effect_win_set(cw, emission, EINA_FALSE, EINA_FALSE, EINA_FALSE);
}

EINTERN void
e_mod_comp_effect_above_wins_set(E_Comp_Win *cw, Eina_Bool show)
{
...
EINTERN void
e_mod_comp_effect_state_set(E_Comp_Effect_Type *type, Eina_Bool state)
{
   E_CHECK(type);
...
e_mod_comp_effect_win_hide(E_Comp_Win *cw)
{
   ...
   vis = e_mod_comp_util_win_visible_get(cw);
                ...
                Eina_Bool   recreate)
{
   ...
   if (ly)
     {
        o = e_mod_comp_effect_object_new(ly, cw, recreate);
        if (o)
          {
   ...
   while ((l = EINA_INLIST_GET(_cw)->prev))
     {
        ...
        o = e_mod_comp_effect_object_new(ly, _cw, recreate);
        if (o)
          {
             evas_object_show(o->img);
             evas_object_show(o->edje);
...
_effect_control_layer_set(E_Comp_Win *cw)
{
   ...
   _MAKE_EMISSION("e,state,visible,on,noeffect");
   e_mod_comp_effect_signal_add(NULL, control_layer_object->edje, emission, "e");
   ...
   if (E_COMP_EFFECT_STYLE_DEFAULT == st)
     {
        ...
        if (launch)
          {
             ...
             if ((cw->depth == 32) && (home_cw) && (_effect_obj_find(home_cw)))
             ...
             else if (vis)
               {
                  ...
                  e_mod_comp_effect_pos_launch_make_emission(cw, emission, sizeof(emission));
             ...
             if (cw->c->keyboard_effect)
               {
                  ...
                  if ((zone) && (zone->rot.block_count))
                    _MAKE_EMISSION("e,state,visible,on,noeffect");
                  else if (e_mod_comp_effect_win_angle_get(cw))
         ...
         */
        else if (TYPE_HOME_CHECK(cw))
          {
             ...
             if (cw2)
               animatable = e_mod_comp_effect_state_get(cw2->eff_type);
             ...
             if (cw2)
               {
                   ...
                   */
                  _effect_below_floating_wins_set(cw2, recreate);
        ...
        if (close)
          {
              ...
              */
             _effect_below_floating_wins_set(cw, recreate);
             ...
             e_mod_comp_effect_pos_close_make_emission(cw, emission, sizeof(emission));
