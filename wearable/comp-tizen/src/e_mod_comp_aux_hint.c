                         ...
                         void      *data2)
{
   ...
   eff.val[0].b = e_mod_comp_effect_state_get(cw->eff_type);
        ...
        if (!hint->deleted)
          {
             if (_STR_CMP(hint->hint, hint_names[AUX_HINT_NAME_EFFECT_ENABLE]))
               ...
               {
                  launch_eff.val[1].i = e_mod_comp_effect_pos_launch_type_get(hint->val);
               ...
               {
                  close_eff.val[1].i = e_mod_comp_effect_pos_close_type_get(hint->val);
   ...
   if ((eff.changed) && (eff.val[0].b != eff.val[1].b))
     e_mod_comp_effect_state_set(cw->eff_type, eff.val[1].b);
