             ...
             if (0 != e_devicemgr.virtual_touchpad_pointed_window)
               {
                  ...
                  if ((pw) && (vw > pw)) e_devicemgr.tmatrix[0] = (float)vw/pw;
                  else if (vw) e_devicemgr.tmatrix[0] = (float)pw/vw;
                  if ((ph) && (vh > ph)) e_devicemgr.tmatrix[4] = (float)vh/ph;
                  else if (vh) e_devicemgr.tmatrix[4] = (float)ph/vh;
        ...
        switch (dev->use)
          {
             case XISlavePointer:
                ...
                if (1==_e_devicemgr_check_device_type(dev->deviceid, E_DEVICEMGR_GAMEPAD, dev->name)) goto handle_keyboard;
...
handle_keyboard:
                ...
                else if (1==_e_devicemgr_check_device_type(dev->deviceid, E_DEVICEMGR_GAMEPAD, dev->name))
                  {
        ...
        for (i=0; i<count; i++)
          {
             if (prop_data[i] == id)
             ...
             else
               {
                  ...
                  if (new_data != NULL) new_data[j] = prop_data[i];
                  j++;
               }
          }
        ecore_x_window_prop_property_set(e_devicemgr.rootWin, e_devicemgr.atomMultitouchDeviceId,
