...
{
   E_ILLUME_VISIBILITY_UNKNOWN = -1, // VisibilityUnknown
...
/* for client window's indicator state */
typedef enum _E_Illume_Indicator_State
{
   E_ILLUME_INDICATOR_STATE_NONE = 0,
   E_ILLUME_INDICATOR_STATE_ON,
   E_ILLUME_INDICATOR_STATE_OFF
} E_Illume_Indicator_State;
   ...
   struct
     {
        ...
        Eina_Bool    use_fixed_ratio;
        double       ratio;
     } resize_req;
   Eina_List *handlers;
   Eina_List *resize_handlers;
   Ecore_Event_Handler *force_render_handler;
   E_Border *border;
   Eina_Bool allow_user_geometry;
   Eina_Bool used_to_floating;
   E_Illume_Indicator_State indicator_state; // Client Window's Indicator Property State
   struct
     {
        struct
          {
             E_Rect geo[4];
             Eina_Bool remember[4];
          } rot;
        struct
          {
             Eina_Bool remember;
...
struct _E_Illume_XWin_Info
{
   ...
   Eina_Bool is_video_overlay : 1;
   Eina_Bool wait_draw : 1;
   Eina_Bool vis_changed: 1;
   Eina_Bool wait_resize;
   Eina_Bool do_not_iconify_by_alpha_opaque : 1; /* _due_to_obscured_by_alpha_opaque */
   E_Illume_Border_Info* bd_info;
   Ecore_X_Window above_xwin;
   Ecore_Timer* transient_timer;
   Eina_Bool transient_ignore;
};
...
void _policy_border_cb_hide(E_Border *bd);
void _policy_border_cb_move(E_Border *bd);
void _policy_border_cb_resize(E_Border *bd);
void _policy_border_cb_comp_src_visibility(E_Border *bd, Eina_Bool visible);
void _policy_border_cb_rotation_change_begin(E_Border *bd);
void _policy_border_cb_rotation_change_end(E_Border *bd);
void _policy_border_cb_desk_layout_set_end(E_Border *bd, int show);
void _policy_zone_layout(E_Zone *zone);
void _policy_zone_move_resize(E_Zone *zone);
void _policy_zone_rotation_change_begin(E_Zone *zone);
...
void _policy_border_eval_end(E_Border *bd);
...
void _policy_border_move(E_Border *bd, int x, int y);
void _policy_border_resize(E_Border *bd, int w, int h);
Ecore_X_Window _policy_active_window_get(Ecore_X_Window root);
EINTERN E_Illume_Border_Info* policy_get_border_info(E_Border *bd);
EINTERN void         policy_border_indicator_control(E_Border *bd);
EINTERN void         policy_border_uniconify_below_borders(E_Border *bd);
EINTERN void         policy_border_illume_handlers_add(E_Illume_Border_Info *bd_info);
EINTERN void         policy_border_illume_handlers_remove(E_Illume_Border_Info *bd_info);
EINTERN void         policy_border_rotation_type_update(E_Border *bd);
EINTERN Eina_Bool    policy_border_is_rot_dependent(E_Border *bd);
EINTERN void         policy_border_dep_rotation_list_add(E_Border *bd);
EINTERN void         policy_border_dep_rotation_list_del(E_Border *bd);
EINTERN Eina_Bool    policy_border_dep_rotation_set(E_Border *bd);
EINTERN E_Illume_XWin_Info* policy_xwin_info_find(Ecore_X_Window win);

/* for locking screen update */
EINTERN void         policy_border_add_block_list(E_Border* bd, Eina_Bool screen_lock);
EINTERN void         policy_border_remove_block_list(E_Border* bd);
EINTERN void         policy_border_visibility_calc_block(Eina_Bool is_block);
#endif
