...
#define LOG_TAG_BORDER "BORDER_WIN_INFO"
...
static void _policy_zone_layout_toast_popup(E_Border *bd, E_Illume_Config_Zone *cz);
...
static void _policy_zone_layout_app_single(E_Illume_Border_Info* bd_info, E_Illume_Config_Zone* cz);
static void _policy_zone_layout_app_dual_top(E_Illume_Border_Info* bd_info, E_Illume_Config_Zone* cz);
static void _policy_zone_layout_app_dual_left(E_Illume_Border_Info* bd_info, E_Illume_Config_Zone* cz);
static void _policy_zone_layout_app_dual_custom(E_Illume_Border_Info* bd_info, E_Illume_Config_Zone* cz);
...
void _policy_border_list_print_by_dlog(Ecore_X_Window win);
...
static void       _policy_active_rotation_bd_update(E_Zone *zone);
...
static void _policy_check_modal_win_clipboard_policy(E_Border *bd, int *x, int *y, int *w, int *h);
...
static void _policy_property_keyboard_state_change(Ecore_X_Event_Window_Property *event);
static Eina_Bool _policy_property_force_render_done(void *data,int type,void *event);
...
static void _policy_property_window_aux_hint_startup_get(E_Border *bd);
static void _policy_property_window_aux_hint_startup_change(Ecore_X_Event_Window_Property *event);
...
/* for supporting split launcher */
static Ecore_X_Atom E_ILLUME_SPLIT_LAUNCHER_WINDOW_SHOW;
...
static Eina_Bool g_block_visibility_calc;

/* for force manual render by deiconify approve */
static Ecore_X_Atom E_ILLUME_ATOM_APPCORE_RECAPTURE_REQUEST;
...
#define TOAST_POPUP_BOTTOM_MARGIN 50

static void
_policy_zone_layout_toast_popup(E_Border *bd, E_Illume_Config_Zone *cz)
{
   E_Border *parent;
   ...
   /* NB : This policy ignores any ICCCM requested positions and centers the
     * popup on it's parent (if it exists) or on the zone
     */
   ...
   if (rotation == 0)
     {
        nx = (bd->zone->w - bd->w) / 2;
        ny = (bd->zone->h - TOAST_POPUP_BOTTOM_MARGIN - bd->h);
        nw = bd->w;
        nh = bd->h;
     }
   else if (rotation == 90)
     {
        nx = (bd->zone->w - TOAST_POPUP_BOTTOM_MARGIN - bd->w);
        ny = (bd->zone->h - bd->h) / 2;
        nw = bd->w;
        nh = bd->h;
     }
     ...
     {
        nx = (bd->zone->w - bd->w) / 2;
        ny = TOAST_POPUP_BOTTOM_MARGIN;
        nw = bd->w;
        nh = bd->h;
     }
   else if (rotation == 270)
     {
        nx = TOAST_POPUP_BOTTOM_MARGIN;
        ny = (bd->zone->h - bd->h) / 2;
        nw = bd->w;
        nh = bd->h;
     }
   ...
   if (resize && move)
     e_border_move_resize(bd, nx, ny, nw, nh);
   else if (resize)
   ...
   /* set layer if needed */
   if (bd->layer != POL_TOAST_POPUP_LAYER)
     e_border_layer_set(bd, POL_TOAST_POPUP_LAYER);
}
   ...
   if (!bd->visible)
     {
        if (e_config->use_tiled_desk_layout)
          {
             if (!(bd->parent &&
                    bd->parent->client.e.state.ly.support &&
                    bd->parent->client.e.state.ly.curr_ly))
               {
                  return;
               }
          }
        ...
        if ((e_config->modal_windows) && (bd->client.netwm.state.modal))
          {
             ...
             ELBF(ELBT_ILLUME, 0, bd->client.win, "Dialog win. old(%d,%d,%d,%d) -> new(%d,%d,%d,%d) by Keyboard", bd->x, bd->y, bd->w, bd->h, nx, ny, mw, mh);

             _policy_check_modal_win_clipboard_policy(bd, &nx, &ny, &mw, &mh);
             ELBF(ELBT_ILLUME, 0, bd->client.win, "Dialog win. old(%d,%d,%d,%d) -> new(%d,%d,%d,%d) by Clipboard", bd->x, bd->y, bd->w, bd->h, nx, ny, mw, mh);
          }
     }
...
_policy_zone_layout_split_launcher(E_Border *bd)
{
   ...
   /* set layer if needed */
   if (bd->layer != POL_SPLIT_LAUNCHER_LAYER)
      e_border_layer_set(bd, POL_SPLIT_LAUNCHER_LAYER);
}

static void
_policy_zone_layout_split_launcher_welcome(E_Border *bd)
{
   if (!bd) return;
...
static void
_policy_zone_layout_mini_app_launcher(E_Border *bd)
{
   if (!bd) return;
...
static void
_policy_zone_layout_cpuinfo(E_Border *bd)
{
   if (!bd->visible) return;

   /* set layer if needed */
   if (bd->layer != POL_CPUINFOL_LAYER)
      e_border_layer_set(bd, POL_CPUINFOL_LAYER);
}
...
_policy_border_add(E_Border *bd)
{
   ...
   else if (e_illume_border_is_assistant_menu(bd))
      policy_assistant_menu_border_add(bd);
...
_policy_border_del(E_Border *bd)
{
   ...
   if (e_illume_border_is_floating(bd))
      ...
      policy_assistant_menu_border_del(bd);
   ...
   if (xwin_info)
     {
        ...
        if (xwin_info->visibility == E_ILLUME_VISIBILITY_UNOBSCURED ||
            xwin_info->visibility == E_ILLUME_VISIBILITY_PARTIALLY_OBSCURED)
          {
             if (_e_illume_cfg->use_indicator_widget)
               {
                  if ((bd->w == bd->zone->w) && (bd->h == bd->zone->h))
        ...
        if (xwin_info->transient_timer)
          {
             ecore_timer_del(xwin_info->transient_timer);
             xwin_info->transient_timer = NULL;
             xwin_info->transient_ignore = EINA_FALSE;
          }
     }
   ...
   if (g_screen_lock_info->is_lock)
     {
        L(LT_SCREEN_LOCK, "[ILLUME2][LOCK] %s(%d)... CAll remove block list. Win:%x\n", __func__, __LINE__, bd->client.win);
        policy_border_remove_block_list(bd);
     }
   ...
   if (dep_rot.active_bd == bd)
     _policy_active_rotation_bd_update(bd->zone);
}
...
_policy_border_post_new_border(E_Border *bd)
{
   ...
   if (bd->new_client)
     {
        /* If we need to change the layer of specific windows,
           we can change its layer in HERE */
        ...
        if (e_illume_border_is_notification(bd))
          {
             ...
             L (LT_NOTIFICATION, "[ILLUME2][NOTIFICATION] %s(%d)... win (0x%07x) is notification window... level = %d\n",
                __func__, __LINE__, bd->client.win, bd_info->level);
                 ...
                 e_illume_border_is_mini_app_launcher(bd))
          {
             if (bd->layer != POL_SPLIT_LAUNCHER_LAYER)
                e_border_layer_set(bd, POL_SPLIT_LAUNCHER_LAYER);
          }
        else if (e_illume_border_is_split_launcher_welcome(bd))
          {
             if (bd->layer != 150)
               {
                  e_border_layer_set(bd, 150);
        ...
        if (ancestor->parent == bd)
          {
             ...
             SLOGI("[WM] Transient_for Error!!! Win:0x%07x and Parent:0x%07x are transient_for each other.", bd->client.win, parent_bd->client.win);
...
static Eina_Bool
_policy_transient_for_timeout(void *data)
{
   E_Border* bd = NULL;
   ...
   if (bd)
     {
        ...
        if (xwin_info)
          {
             xwin_info->transient_ignore = EINA_TRUE;
             xwin_info->transient_timer = NULL;
          }
     }
...
_policy_border_pre_fetch(E_Border *bd)
{
   ...
   if (bd->new_client)
     {
        ...
        policy_assistant_menu_window_state_change(bd, state);
        ...
        _policy_property_window_aux_hint_startup_get(bd);
     }
   ...
   // since we classify the type of rotation using name and class for now,
   // we should update the type every time the name and class changed.
   // delay type update to post_fetch, because name and class will be changed in _e_border_eval0.
   ...
   if (bd->client.icccm.fetch.transient_for)
     {
        ...
        SLOGI("[ILLUME2][TRANSIENT_FOR] win:0x%07x, parent:0x%07x\n", bd->client.win, bd->client.icccm.transient_for);
        if (bd_parent)
          {
         ...
         */
          ...
          {
             if (xwin_info && xwin_info->transient_ignore)
                goto transient_fetch_done;
             else
...
transient_fetch_fail:
        if (xwin_info && !xwin_info->transient_timer)
          {
             xwin_info->transient_timer = ecore_timer_add(2.0, _policy_transient_for_timeout, bd);
          }
        return;
     }
}
...
_policy_border_post_assign(E_Border *bd)
{
   ...
   if (e_illume_border_is_utility (bd) ||
       e_illume_border_is_toast_popup(bd))
     {
        E_Illume_Border_Info *bd_info = policy_get_border_info(bd);
        if (bd_info) bd_info->allow_user_geometry = EINA_TRUE;
     }
   ...
   if (e_illume_border_is_keyboard(bd))
     {
        ...
        EINA_LIST_FOREACH(bd->transients, l, child)
          {
             if (e_illume_border_is_clipboard(child))
               {
                  if (!e_object_is_del(E_OBJECT(child)))
                    {
                       e_border_hide(child, 0);
                       e_object_del(E_OBJECT(child));
                       ecore_x_window_hide(child->client.win);
                    }
               }
          }
...
_policy_border_cb_move(E_Border *bd)
{
   ...
   if (e_illume_border_is_lock_screen(bd))
   {
        if (!E_CONTAINS(bd->zone->x, bd->zone->y, bd->zone->w, bd->zone->h, bd->x, bd->y, bd->w, bd->w))
          {
             e_illume_util_mem_trim();
          }
   }
...
_policy_border_cb_rotation_change_end(E_Border *bd)
{
   ...
   if (!bd) return;
    ...
    if (e_illume_border_is_floating(bd) || e_illume_border_is_assistant_menu(bd))
     {
...
void
_policy_border_cb_desk_layout_set_end(E_Border *bd, int show)
{
   E_Illume_Border_Info *bd_info = NULL;
   int rot;
   if (!bd) return;
   ...
   if (e_illume_border_is_split_launcher(bd)) return;

   E_Border_Aux_Hint *hint;
   Eina_List *l;
   EINA_LIST_FOREACH(bd->client.e.state.aux_hint.hints, l, hint)
     {
        if (strcmp(hint->hint, "wm.policy.win.startup.by") == 0)
          {
             eina_stringshare_del(hint->val);
             hint->val = eina_stringshare_add("1");
             break;
          }
     }
   ...
   if (bd->client.e.state.rot.type == E_BORDER_ROTATION_TYPE_DEPENDENT)
     policy_border_dep_rotation_set(bd);
   else if (bd->client.e.state.rot.type == E_BORDER_ROTATION_TYPE_NORMAL)
     {
        rot = e_border_rotation_recommend_angle_get(bd);

        if ((rot != -1) &&
            (rot != e_border_rotation_curr_angle_get(bd)) &&
            (rot != e_border_rotation_next_angle_get(bd)))
          e_border_rotation_set(bd, rot);
     }
   ...
   /* for screen lock */
   if (bd->client.e.state.ly.curr_ly != bd_info->curr_ly)
     {
        if ((bd->client.e.state.ly.curr_ly == NULL) ||
            (bd_info->curr_ly == NULL))
          {
             if (bd->visibility == 0)
               {
                  L(LT_SCREEN_LOCK, "[ILLUME2][LOCK] %s(%d)... CAll add block list with Screen Lock. Win:%x visible:%d, visibility:%d\n", __func__, __LINE__, bd->client.win, bd->visible, bd->visibility);
                  policy_border_add_block_list(bd, EINA_TRUE);
               }
             else
               {
                  if (show)
                    {
                       L(LT_SCREEN_LOCK, "[ILLUME2][LOCK] %s(%d)... CAll add block list. Win:%x visible:%d, visibility:%d\n", __func__, __LINE__, bd->client.win, bd->visible, bd->visibility);
                       policy_border_add_block_list(bd, EINA_FALSE);
                    }
               }
          }
        else
          {
             if ((bd->client.e.state.ly.curr_ly->w != bd_info->curr_ly->w) ||
                 (bd->client.e.state.ly.curr_ly->h != bd_info->curr_ly->h))
               {
                  if (bd->visibility == 0)
                    {
                       L(LT_SCREEN_LOCK, "[ILLUME2][LOCK] %s(%d)... CAll add block list with Screen Lock. Win:%x visible:%d, visibility:%d\n", __func__, __LINE__, bd->client.win, bd->visible, bd->visibility);
   ...
   bd_info->curr_ly = bd->client.e.state.ly.curr_ly;
   return;
}
   ...
   EINA_LIST_FOREACH(e_border_info_list, l, bd_info)
     {
        ...
        else if (e_illume_border_is_toast_popup(bd))
           _policy_zone_layout_toast_popup(bd, cz);
        ...
        /* are we laying out a dialog ? */
           ...
           _policy_zone_layout_split_launcher_welcome(bd);

        else if (e_illume_border_is_mini_app_launcher(bd))
           _policy_zone_layout_mini_app_launcher(bd);

        else if (e_illume_border_is_cpuinfo(bd))
           _policy_zone_layout_cpuinfo(bd);
             ...
             if (!cz->mode.dual)
               {
                  /* for desktop mode */
                  if (E_ILLUME_BORDER_IS_IN_MOBILE(bd))
                    _policy_zone_layout_app_single(bd_info, cz);
                  else if (E_ILLUME_BORDER_IS_IN_DESKTOP(bd))
                  ...
                  if (cz->mode.side == 0)
                    {
                       ...
                       if (ty <= bd->zone->y)
                          _policy_zone_layout_app_dual_top(bd_info, cz);
                       else
                          _policy_zone_layout_app_dual_custom(bd_info, cz);
                    }
                  else
                     _policy_zone_layout_app_dual_left(bd_info, cz);
               }
          }
        ...
        if (e_illume_border_is_floating(bd))
           ...
           policy_assistant_menu_zone_layout_assistant_menu(bd);
...
_policy_resize_start(E_Illume_Border_Info *bd_info)
{
   ...
   if(!(edje_object_file_set(o, buf, "new_shadow"))
      || (bd->client.illume.win_state.state == ECORE_X_ILLUME_WINDOW_STATE_NORMAL))
     {
...
#ifdef _E_USE_EFL_ASSIST_
   memset(buf, 0, sizeof(buf));
   snprintf(buf, sizeof(buf), "%s/e-module-illume2-tizen-ChangeableColorInfo.xml", _e_illume_mod_dir);
   table = ea_theme_color_table_new(buf);
   if (table)
     {
        ea_theme_colors_set(table, EA_THEME_STYLE_DARK);
        if (ea_theme_color_get("AM007", &r, &g, &b, &a, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL))
   ...
   if (bd_info->resize_req.use_fixed_ratio)
     {
        bd_info->resize_req.ratio = (double)bd->h / (double)bd->w; // w:h = 1:ratio
     }
}
...
static void _policy_property_rotate_win_angle_change (Ecore_X_Event_Window_Property *event)
{
   ...
   if (xwin_info)
     {
        if ((xwin_info->visibility == E_ILLUME_VISIBILITY_UNOBSCURED) ||
            (xwin_info->visibility == E_ILLUME_VISIBILITY_UNKNOWN))
          {
...
static void _policy_active_win_change(E_Illume_XWin_Info *xwin_info, Ecore_X_Window active_win)
{
   ...
   if (active_win != g_active_win)
     {
        ...
        if (active_bd)
          {
             SLOGI("[WM] ACT WIN 0x%07x(%d) -> 0x%07x(%d)", g_active_win, g_active_pid, active_win, active_pid);
 
             if ((E_ILLUME_BORDER_IS_IN_DESKTOP(active_bd)) ||
                 (active_bd->client.illume.win_state.state != ECORE_X_ILLUME_WINDOW_STATE_NORMAL))
               {
                  e_border_raise(active_bd);
               }

             if ((active_bd->parent) &&
                 (!active_bd->client.e.state.ly.support) &&
                 (e_config->use_tiled_desk_layout))
               {
                  active_bd_parent = active_bd->parent;
                  while ((!active_bd_parent->client.e.state.ly.support) &&
                         (active_bd_parent->parent))
                    {
                       active_bd_parent = active_bd_parent->parent;
                    }

                  if (active_bd_parent->client.e.state.ly.support)
                    {
                       if (active_bd_parent->client.e.state.ly.curr_ly)
                         {
                            e_desk_active_desk_layout_set
                               (active_bd_parent->client.e.state.ly.curr_ly->desk,
                                active_bd_parent->client.e.state.ly.curr_ly);
                         }
                    }
               }
          }
   ...
   /* change activated E_Desk_Layout */
   if (e_config->use_tiled_desk_layout)
     {
        if (active_bd)
          {
             if ((active_bd->client.e.state.ly.support) &&
                 (active_bd->client.e.state.ly.curr_ly))
               {
                  e_desk_active_desk_layout_set
                     (active_bd->client.e.state.ly.curr_ly->desk,
                      active_bd->client.e.state.ly.curr_ly);
               }
          }
        else
             ...
             if (zone)
               {
                  ...
                  if ((desk) && (desk->ly.tiled))
                    {
                       e_desk_active_desk_layout_set(desk, NULL);
                    }
               }
          }
     }
...
static void _policy_property_active_win_change (Ecore_X_Event_Window_Property *event)
{
   ...
   if (active_bd)
     {
        xwin_info = policy_xwin_info_find(active_bd->win);
     }
   else
     {
        xwin_info = policy_xwin_info_find(active_win);
     }
...
static void _policy_property_window_opaque_change (Ecore_X_Event_Window_Property *event)
{
   ...
   E_Illume_XWin_Info* xwin_info = policy_xwin_info_find(bd->win);
   if (xwin_info)
     {
        xwin_info->vis_changed = EINA_TRUE;
     }
}
...
policy_border_illume_handlers_remove(E_Illume_Border_Info *bd_info)
{
   ...
   if (bd_info->force_render_handler)
      ecore_event_handler_del(bd_info->force_render_handler);

   _policy_border_floating_resize_handlers_remove(bd_info);
}

static void
_fixed_ratio_geometry_get(E_Illume_Border_Info *bd_info,
                          ...
                          E_Resizable_Area_Info *area)
{
   ...
   int min_w, min_h;
   int max_w, max_h;
   double w_base_ratio, h_base_ratio;
   ...
   w_base_ratio = bd_info->resize_req.ratio; // h/w
   h_base_ratio = 1 / bd_info->resize_req.ratio; // w/h
   ...
   switch (bd_info->resize_req.angle)
     {
      case 90:
         ...
         switch (direction)
           {
            case ECORE_X_NETWM_DIRECTION_SIZE_T:
            case ECORE_X_NETWM_DIRECTION_SIZE_TL:
            case ECORE_X_NETWM_DIRECTION_SIZE_TR:
               ev_x -= bd_info->resize_req.mouse.knob_dx;
               break;

            case ECORE_X_NETWM_DIRECTION_SIZE_B:
            case ECORE_X_NETWM_DIRECTION_SIZE_BL:
            case ECORE_X_NETWM_DIRECTION_SIZE_BR:
               ev_x += bd_info->resize_req.mouse.knob_dx;
               break;
            default:
               break;
         ...
         switch (direction)
           {
            case ECORE_X_NETWM_DIRECTION_SIZE_R:
            case ECORE_X_NETWM_DIRECTION_SIZE_TR:
            case ECORE_X_NETWM_DIRECTION_SIZE_BR:
               ev_y -= bd_info->resize_req.mouse.knob_dy;
               break;

            case ECORE_X_NETWM_DIRECTION_SIZE_L:
            case ECORE_X_NETWM_DIRECTION_SIZE_TL:
            case ECORE_X_NETWM_DIRECTION_SIZE_BL:
               ev_y += bd_info->resize_req.mouse.knob_dy;
   ...
   if (bd->w > bd->h)
     {
        // MIN
        temp_w = (double)min_h * h_base_ratio;
        if (min_w > temp_w)
          {
             min_w = min_w;
             min_h = (double)min_w * w_base_ratio;
          }
        else
          {
             min_w = temp_w;
        ...
        // MAX
        max_w = limit;
        max_h = (double)limit * w_base_ratio;
     }
   else
     {
        // MIN
        temp_h = (double)min_w * w_base_ratio;
        if (min_h > temp_h)
          {
             min_w = (double)min_h * h_base_ratio;
             min_h = min_h;
          }
        else
          {
        ...
        // MAX
        max_w = (double)limit * h_base_ratio;
        max_h = limit;
     }

   // check whether bd's size is smaller than the min size
   if ((min_w > bd->w) || (min_h > bd->h))
     {
        switch (bd_info->resize_req.angle)
          {
           case 0:
           ...
           case 90:
              ...
              base_h = min_h;
              base_x = bd->x;
              base_y = bd->y - (min_h - bd->h);
              break;

           case 180:
              base_w = min_w;
              base_h = min_h;
              base_x = bd->x - (min_w - bd->w);
              base_y = bd->y - (min_h - bd->h);
              break;
   ...
   if (direction == ECORE_X_NETWM_DIRECTION_SIZE_BR)
     {
        switch (bd_info->resize_req.angle)
          {
           case 0: // G_BR
              CALCULATE_RESIZE_BR(ev_x, ev_y, base_x, base_y, base_w, base_h, w_base_ratio, h_base_ratio, dx, dy, cx, cy, x1, y1, x2, y2);
              break;
           case 90: // G_TR
              CALCULATE_RESIZE_TR(ev_x, ev_y, base_x, base_y, base_w, base_h, w_base_ratio, h_base_ratio, dx, dy, cx, cy, x1, y1, x2, y2);
              break;
           case 180: // G_TL
              CALCULATE_RESIZE_TL(ev_x, ev_y, base_x, base_y, base_w, base_h, w_base_ratio, h_base_ratio, dx, dy, cx, cy, x1, y1, x2, y2);
              break;
           case 270: // G_BL
              CALCULATE_RESIZE_BL(ev_x, ev_y, base_x, base_y, base_w, base_h, w_base_ratio, h_base_ratio, dx, dy, cx, cy, x1, y1, x2, y2);
              break;
           default:
              break;
          }
   ...
   else if (direction == ECORE_X_NETWM_DIRECTION_SIZE_T)
     {
        switch (bd_info->resize_req.angle)
          {
           case 0: // G_T (similar G_TR)
              CALCULATE_RESIZE_T(ev_x, ev_y, base_x, base_y, base_w, base_h, w_base_ratio, h_base_ratio, dx, dy, cx, cy, x1, y1, x2, y2);
              break;
           case 90: // G_L (similar G_TL)
              CALCULATE_RESIZE_L(ev_x, ev_y, base_x, base_y, base_w, base_h, w_base_ratio, h_base_ratio, dx, dy, cx, cy, x1, y1, x2, y2);
              break;
           case 180: // G_B (similar G_BL)
              CALCULATE_RESIZE_B(ev_x, ev_y, base_x, base_y, base_w, base_h, w_base_ratio, h_base_ratio, dx, dy, cx, cy, x1, y1, x2, y2);
              break;
           case 270: // G_R (similar G_BR)
              CALCULATE_RESIZE_R(ev_x, ev_y, base_x, base_y, base_w, base_h, w_base_ratio, h_base_ratio, dx, dy, cx, cy, x1, y1, x2, y2);
              break;
           default:
              break;
          }
   ...
   if (area)
     {
        area->x_dist = w;
        area->y_dist = h;
        area->min_width = min_w;
        area->min_height = min_h;
                   ...
                   if (bd->w > bd->h)
                     {
                        ...
                        area->max_height = (double)max_size * w_base_ratio;
                     }
                   else
                     {
                        area->max_width = (double)max_size * h_base_ratio;
   ...
   if (x1 > base_x) x1 = cx - w;
   if (y1 > base_y) y1 = cy - h;
...
static void
_free_ratio_geometry_get(E_Illume_Border_Info *bd_info,
                         Evas_Coord_Rectangle *r,
                         int                   ev_x,
                         int                   ev_y,
                         int                   direction,
                          ...
                          E_Resizable_Area_Info *area)
{
   ...
   if (bd_info->resize_req.use_fixed_ratio)
     _fixed_ratio_geometry_get(bd_info, r, ev_x, ev_y, direction, area);
   else
     _free_ratio_geometry_get(bd_info, r, ev_x, ev_y, direction, area);
}
...
_policy_property_change(Ecore_X_Event_Window_Property *event)
{
   ...
   else if (event->atom == E_WINDOW_AUX_HINT_STARTUP)
     {
        _policy_property_window_aux_hint_startup_change(event);
     }
   else if (event->atom == ECORE_X_ATOM_E_VIRTUAL_KEYBOARD_STATE)
     {
        _policy_property_keyboard_state_change(event);
     }
}
...
_policy_border_list_print (Ecore_X_Window win)
{
   ...
   _policy_border_list_print_by_dlog(win);
   ...
   if ((info.type & PT_STACK) == PT_STACK)
     {
        ...
        fprintf (out, " No  Border     ClientWin     w     h       x       y   layer  visible visibility curr_ly  WinName\n" );
        fprintf (out, "-----------------------------------------------------------------------------------------------------\n" );
        ...
        if (bl)
          {
             while ((bd = e_container_border_list_prev(bl)))
               {
                  fprintf (out, "%3i  0x%07x  0x%07x  %4i  %4i  %6i  %6i  %5i  %5i   %7c    %5i     %-30s \n",
                           i++, bd->win, bd->client.win, bd->w, bd->h, bd->x, bd->y, bd->layer, bd->visible, bd->visibility ? 'X':'O', bd->client.e.state.ly.curr_ly ? bd->client.e.state.ly.curr_ly->id:-1, _policy_border_name_get(bd));
               }
        ...
        if (xwin_info_list)
          {
             ...
             fprintf (out, " No  Border     ClientWin     w     h       x       y  depth layer view  visibi  comp icon  by_ skip_   WinName\n" );
             fprintf (out, "                                                                   able  lity    _vis       wm  iconic\n" );
             fprintf (out, "-------------------------------------------------------------------------------------------------------------------------------\n" );
                       ...
                       if (xwin_info->bd_info->border)
                         {
                            ...
                            fprintf (out, "%3i  0x%07x  0x%07x  %4i  %4i  %6i  %6i  %4i  %4i  %3i   %4c    %3i  %3i   %2i  %3i     %-30s\n",
                                    i++, bd->win, bd->client.win, xwin_info->attr.w, xwin_info->attr.h, xwin_info->attr.x, xwin_info->attr.y, xwin_info->attr.depth, bd->layer,
                                    xwin_info->viewable, xwin_info->visibility ? 'X':'O', xwin_info->comp_vis, xwin_info->bd_info->border->iconic, xwin_info->iconify_by_wm, xwin_info->skip_iconify, _policy_border_name_get(bd));
                         }
                    }
               }
             fprintf (out, "-------------------------------------------------------------------------------------------------------------------------------\n" );
        ...
        Eina_List *uniconify_pending = e_border_uniconify_pending_list_get();
        pend_bd = NULL;

        if (uniconify_pending)
          {
             fprintf(out, "uniconify pending list : ");
             EINA_LIST_FOREACH(uniconify_pending, l, pend_bd)
               {
                  fprintf(out, "0x%07x -> ", pend_bd->client.win);
               }
             fprintf(out, "NULL\n");
        ...
        Eina_List *uniconify_waiting = e_border_uniconify_waiting_list_get();
        if (uniconify_waiting)
          {
             fprintf(out, "uniconify waiting list : ");
             EINA_LIST_FOREACH(uniconify_waiting, l, pend_bd)
               {
                  fprintf(out, "0x%07x -> ", pend_bd->client.win);
               }
             fprintf(out, "NULL\n");
        ...
        fprintf (out, "---------------------------------------------------------------------------------------------\n" );
        E_Desk *desk = e_desk_current_get(temp_bd->zone);
        E_Desk_Layout *ly = NULL;
        i = 0;
        if (desk)
          {
             EINA_INLIST_FOREACH(desk->ly.list, ly)
               {
                  fprintf (out, "%3i  %4i  %4i  %6i  %6i  \n",
                           i++, ly->w, ly->h, ly->x, ly->y);
               }
          }
        ...
        fprintf (out, "-------------------------------- SPLIT support LIST -------------------------------------------------\n" );
        fprintf (out, " No  Border     ClientWin     w     h       x       y  sup  first  by  to  curr_ly  prev_ly  WinName\n" );
        fprintf (out, "-----------------------------------------------------------------------------------------------------\n" );
        i = 1;
        bl = e_container_border_list_last(temp_bd->zone->container);
        if (bl)
          {
             while ((bd = e_container_border_list_prev(bl)))
               {
                  if (!bd->client.e.state.ly.support) continue;
                  fprintf (out, "%3i  0x%07x  0x%07x  %4i  %4i  %6i  %6i   %i  %4i  %3i  %3i  %5i    %5i    %-30s \n",
                           i++, bd->win, bd->client.win, bd->w, bd->h, bd->x, bd->y, bd->client.e.state.ly.support,
                           bd->new_client, bd->launch_info.launch_by, bd->launch_info.launch_to,
                           bd->client.e.state.ly.curr_ly ? bd->client.e.state.ly.curr_ly->id:-1,
                           bd->client.e.state.ly.prev_ly ? bd->client.e.state.ly.prev_ly->id:-1,
                           _policy_border_name_get(bd));
               }
             e_container_border_list_free(bl);
          }
        fprintf (out, "-----------------------------------------------------------------------------------------------------\n\n");

        fprintf (out, "-------------------------------- BD IN DESK LAYOUT INFO--------------------------------------\n" );
        int k = 0;
        i = 0;
        if (desk)
          {
                  ...
                  if (bl)
                    {
                       while ((bd = e_container_border_list_prev(bl)))
                         {
                            if (bd->client.e.state.ly.curr_ly == ly)
                              {
                                 fprintf (out, "%3i  0x%07x  0x%07x  %4i  %4i  %6i  %6i  %5i  %5i     %-30s \n",
                                          k++, bd->win, bd->client.win, bd->w, bd->h, bd->x, bd->y, bd->layer,
                                          bd->visible,  _policy_border_name_get(bd));
                              }
                         }
                       e_container_border_list_free(bl);
                    }
...
finish:
   ...
   if (g_screen_lock_info->is_lock)
     {
        fprintf (out, "---------------------------------------------------------------------------------------------\n");
        fprintf (out, "Locked:%d, Block_Screen_Update:%d\n", g_screen_lock_info->is_lock, g_screen_lock_info->is_block_scr_update);
...
void
_policy_border_list_print_by_dlog(Ecore_X_Window win)
{
   Eina_List* border_list;
   Eina_List *l;
   E_Border *bd;
   ...
   if ((info.type & PT_STACK) == PT_STACK)
     {
        ...
        if (border_list)
          {
             i = 1;
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,"--------------------------------BORDER INFO--------------------------------------------------" );
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, " No  Border     ClientWin     w     h       x       y   layer  visible  WinName\n" );
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "---------------------------------------------------------------------------------------------" );
             ...
             EINA_LIST_FOREACH(border_list, l, bd)
               {
                  ...
                  __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "%3i  0x%07x  0x%07x  %4i  %4i  %6i  %6i  %5i  %5i     %-30s ",
                          i++, bd->win, bd->client.win, bd->w, bd->h, bd->x, bd->y, bd->layer, bd->visible, _policy_border_name_get(bd));
               }
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "---------------------------------------------------------------------------------------------" );
          }
        ...
        if (e_border_info_list)
          {
             i = 1;
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "--------------------------------ILLUME BORDER INFO-----------------------------------------------------------------------------" );
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, " No  Border     ClientWin     w     h       x       y   isKBD layer level desk   WinName                INDICATOR_STATE" );
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "-------------------------------------------------------------------------------------------------------------------------------" );
             ...
             EINA_LIST_FOREACH(e_border_info_list, l, bd_info)
               {
                  ...
                  __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "%3i  0x%07x  0x%07x  %4i  %4i  %6i  %6i  %3i  %5i %5i   (%d,%d)  %-30s             %s",
                          i++, bd_info->border->win, bd_info->border->client.win, bd_info->border->w, bd_info->border->h, bd_info->border->x, bd_info->border->y,
                          bd_info->border->client.vkbd.vkbd, bd_info->border->layer, bd_info->level, desk_x, desk_y, _policy_border_name_get(bd), buf);
               }
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "-------------------------------------------------------------------------------------------------------------------------------" );
        ...
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "-------------------------------- E17 STACK INFO------------------------------------------------------" );
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER," No  Border     ClientWin     w     h       x       y   layer  visible visibility curr_ly  WinName" );
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "-----------------------------------------------------------------------------------------------------" );
        ...
        while ((bd = e_container_border_list_prev(bl)))
          {
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "%3i  0x%07x  0x%07x  %4i  %4i  %6i  %6i  %5i  %5i   %7c    %5i     %-30s ",
                     i++, bd->win, bd->client.win, bd->w, bd->h, bd->x, bd->y, bd->layer, bd->visible, bd->visibility ? 'X':'O', bd->client.e.state.ly.curr_ly ? bd->client.e.state.ly.curr_ly->id:-1, _policy_border_name_get(bd));
          }
        e_container_border_list_free(bl);
        ...
        if (xwin_info_list)
          {
             i = 1;
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,"--------------------------------ILLUME STACK INFO------------------------------------------------------------------------------" );
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, " No  Border     ClientWin     w     h       x       y  depth layer view  visibi  comp icon  by_ skip_   WinName" );
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "                                                                   able  lity    _vis       wm  iconic" );
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "-------------------------------------------------------------------------------------------------------------------------------" );
                       ...
                       if (xwin_info->bd_info->border)
                         {
                            ...
                            __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "%3i  0x%07x  0x%07x  %4i  %4i  %6i  %6i  %4i  %4i  %3i   %4c    %3i  %3i   %2i  %3i     %-30s",
                                    i++, bd->win, bd->client.win, xwin_info->attr.w, xwin_info->attr.h, xwin_info->attr.x, xwin_info->attr.y, xwin_info->attr.depth, bd->layer,
                                    xwin_info->viewable, xwin_info->visibility ? 'X':'O', xwin_info->comp_vis, xwin_info->bd_info->border->iconic, xwin_info->iconify_by_wm, xwin_info->skip_iconify, _policy_border_name_get(bd));
                         }
                    }
        ...
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "-------------------------------- PENDING INFO -----------------------------------------------" );
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, " No  Border     ClientWin   hide  lower  destroy  done  list" );
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "---------------------------------------------------------------------------------------------" );
        E_Border *pend_bd = NULL;
        i = 1;
        bl = e_container_border_list_last(temp_bd->zone->container);
             ...
             while ((bd = e_container_border_list_prev(bl)))
               {
                  if (bd->client.e.state.pending_event.pending)
                    {
                       __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "%3i  0x%07x  0x%07x  %4i  %4i  %6i  %6i   ",
                                    i++, bd->win, bd->client.win,
                                    bd->client.e.state.pending_event.hide.pending,
                                    bd->client.e.state.pending_event.lower.pending,
                                    bd->client.e.state.pending_event.destroy.pending,
                       ...
                       EINA_LIST_FOREACH(bd->client.e.state.pending_event.wait_for_list, l, pend_bd)
                         {
                            __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "0x%07x  ", pend_bd->client.win);
                         }
                       __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "\n");
                    }
               }
       ...
       __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "---------------------------------------------------------------------------------------------\n" );
        ...
        if (uniconify_pending)
          {
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "uniconify pending list : ");
             EINA_LIST_FOREACH(uniconify_pending, l, pend_bd)
               {
                  __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "0x%07x -> ", pend_bd->client.win);
               }
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "NULL");
          }

        Eina_List *uniconify_waiting = e_border_uniconify_waiting_list_get();
        if (uniconify_waiting)
          {
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "uniconify waiting list : ");
             EINA_LIST_FOREACH(uniconify_waiting, l, pend_bd)
               {
                  __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "0x%07x -> ", pend_bd->client.win);
               }
        ...
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "-------------------------------- DESK LAYOUT INFO--------------------------------------------" );
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, " No   w     h         x       y   " );
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "---------------------------------------------------------------------------------------------" );
        E_Desk *desk = e_desk_current_get(temp_bd->zone);
        E_Desk_Layout *ly = NULL;
        i = 0;
        if (desk)
          {
             EINA_INLIST_FOREACH(desk->ly.list, ly)
               {
                  __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "%3i  %4i  %4i  %6i  %6i  ",
                               i++, ly->w, ly->h, ly->x, ly->y);
               }
          }
        ...
        if (man)
          {
             ...
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "---------------------------------------------------------------------------------------------" );
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "[CONTROL LAYER VISIBILITY] %s\n", ctr_vis? "ON":"OFF");
          }

        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "-------------------------------- SPLIT support LIST -------------------------------------------------" );
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, " No  Border     ClientWin     w     h       x       y  sup  first  by  to  curr_ly  prev_ly  WinName" );
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "-----------------------------------------------------------------------------------------------------" );
        i = 1;
        bl = e_container_border_list_last(temp_bd->zone->container);
        if (bl)
          {
             while ((bd = e_container_border_list_prev(bl)))
               {
                  if (!bd->client.e.state.ly.support) continue;
                  __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "%3i  0x%07x  0x%07x  %4i  %4i  %6i  %6i   %i  %4i  %3i  %3i  %5i    %5i    %-30s ",
                               i++, bd->win, bd->client.win, bd->w, bd->h, bd->x, bd->y, bd->client.e.state.ly.support,
                               bd->new_client, bd->launch_info.launch_by, bd->launch_info.launch_to,
                               bd->client.e.state.ly.curr_ly ? bd->client.e.state.ly.curr_ly->id:-1,
                               bd->client.e.state.ly.prev_ly ? bd->client.e.state.ly.prev_ly->id:-1,
        ...
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "-------------------------------- BD IN DESK LAYOUT INFO--------------------------------------" );
        int k = 0;
        i = 0;
        if (desk)
          {
             EINA_INLIST_FOREACH(desk->ly.list, ly)
               {
                  __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "LAYOUT%02d ------------------------------------------------------------------------------------", i++);
                  __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, " No  Border     ClientWin     w     h       x       y   layer  visible  WinName" );
                  bl = e_container_border_list_last(temp_bd->zone->container);
                  if (bl)
                    {
        ...
        if (xwin_info_list)
          {
             i = 1;
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "--------------------------------BORDER INFO--------------------------------------------------" );
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  " No  Win          w     h       x       y   depth  viewable  visibility comp_vis iconify by_wm skip_iconic is_border(Client Win)" );
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "---------------------------------------------------------------------------------------------" );
                  ...
                  if (xwin_info->bd_info)
                    {
                       if (xwin_info->bd_info->border)
                         {
                            __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "%3i  0x%07x  %4i  %4i  %6i  %6i  %5i   %5i      %5i     %5i   %5i  %5i   %5i   yes(0x%07x)",
                                    i++, xwin_info->id, xwin_info->attr.w, xwin_info->attr.h, xwin_info->attr.x, xwin_info->attr.y, xwin_info->attr.depth,
                                    xwin_info->viewable, xwin_info->visibility, xwin_info->comp_vis, xwin_info->bd_info->border->iconic, xwin_info->iconify_by_wm, xwin_info->skip_iconify, xwin_info->bd_info->border->client.win);
                         }
                       else
                         {
                            __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "%3i  0x%07x  %4i  %4i  %6i  %6i  %5i   %5i     %5i      %5i       0    %3i   %5i     no(NULL)",
                                    i++, xwin_info->id, xwin_info->attr.w, xwin_info->attr.h, xwin_info->attr.x, xwin_info->attr.y, xwin_info->attr.depth,
                                    xwin_info->viewable, xwin_info->visibility, xwin_info->comp_vis, xwin_info->iconify_by_wm, xwin_info->skip_iconify);
                         }
                    }
...
finish:
   ...
   __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER, "--------------------------------GLOBAL INFO--------------------------------------------------" );
   __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "g_rotated_win:0x%07x (g_root_angle:%d)", g_rotated_win, g_root_angle);
   __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "g_active_win:0x%07x (pid:%d, angle:%d)", g_active_win, g_active_pid, _policy_window_rotation_angle_get(g_active_win));
   __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "e_focused_win:0x%07x", focused_bd ? focused_bd->client.win : (unsigned int)NULL);
   __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "g_indi_control_win:0x%07x", g_indi_control_win);
   __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "indicator cmd win:0x%x", indi_info.cmd_win);
   __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "active indicator win:0x%x", indi_info.active_win);
   __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "---------------------------------------------------------------------------------------------" );
   if (g_screen_lock_info->is_lock)
     {
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "---------------------------------------------------------------------------------------------");
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "Locked:%d, Block_Screen_Update:%d\n", g_screen_lock_info->is_lock, g_screen_lock_info->is_block_scr_update);
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "Screen Lock Info : ");
        EINA_LIST_FOREACH(g_screen_lock_info->blocked_list, l, bd)
          {
             __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "0x%07x  ", bd->client.win);
          }
        __dlog_print(LOG_ID_SYSTEM, DLOG_DEBUG, LOG_TAG_BORDER,  "\n---------------------------------------------------------------------------------------------");
     }
}
...
int _policy_atom_init (void)
{
   ...
   E_ILLUME_SPLIT_LAUNCHER_WINDOW_SHOW = ecore_x_atom_get("_NET_WM_WINDOW_SHOW");
   if (!E_ILLUME_SPLIT_LAUNCHER_WINDOW_SHOW)
     {
        fprintf(stderr, "[ILLUME2] Critical Error!!! Cannot create _NET_WM_WINDOW_SHOW Atom...\n");
        return 0;
     }

   E_ILLUME_ATOM_APPCORE_RECAPTURE_REQUEST = ecore_x_atom_get("_E_ILLUME_ATOM_APPCORE_RECAPTURE_REQUEST");
   if (!E_ILLUME_ATOM_APPCORE_RECAPTURE_REQUEST)
     {
        fprintf(stderr, "[ILLUME2] Critical Error!!! Cannot create _E_ILLUME_ATOM_APPCORE_RECAPTURE_REQUEST Atom...\n");
        return 0;
     }
...
int _policy_init (void)
{
   ...
   g_screen_lock_info->is_lock = EINA_FALSE;
   g_screen_lock_info->is_block_scr_update = EINA_FALSE;
   ...
   policy_assistant_menu_init();
   _policy_aux_hint_init();
...
void _policy_fin (void)
{
   ...
   if (g_screen_lock_info->is_block_scr_update)
     {
   ...
   policy_assistant_menu_shutdown();
}
...
_policy_zone_layout_app_layer_check (E_Border* bd)
{
   ...
   if (e_illume_border_is_volume(bd))
     {
        layer = POL_VOLUME_LAYER;
     }

   if (e_illume_border_is_assistant_menu(bd))
     {
        layer = POL_ASSISTANT_MENU_LAYER;
     }
...
static void
_policy_zone_layout_app_single(E_Illume_Border_Info* bd_info, E_Illume_Config_Zone *cz)
{
   E_Border* bd;
   E_Border *child;
   Eina_List *l;
   ...
   if (bd->client.illume.win_state.state)
     ...
     {
        Eina_Bool ly_policy = EINA_FALSE;
        if (e_config->use_tiled_desk_layout)
          {
             if (bd->client.e.state.ly.support)
               {
                  if (bd->client.e.state.ly.curr_ly)
                    {
                       E_Desk_Layout *ly = bd->client.e.state.ly.curr_ly;
                       if ((bd->w != ly->w) || (bd->h != ly->h))
                         resize = EINA_TRUE;

                       if ((bd->x != ly->x) || (bd->y != ly->y))
                         move = EINA_TRUE;

                       if (resize && move)
                         e_border_move_resize(bd, ly->x, ly->y, ly->w, ly->h);
                       else if (resize)
                         e_border_resize(bd, ly->w, ly->h);
                       else if (move)
                         e_border_move(bd, ly->x, ly->y);
...
static void
_policy_zone_layout_app_dual_top(E_Illume_Border_Info* bd_info, E_Illume_Config_Zone *cz)
{
   E_Border* bd;
   E_Border* temp_bd;
   int ny, nh;
...
static void
_policy_zone_layout_app_dual_left(E_Illume_Border_Info* bd_info, E_Illume_Config_Zone *cz)
{
   E_Border* bd;
   E_Border* temp_bd;
   int ky, kh, nx, nw;
...
static void
_policy_zone_layout_app_dual_custom(E_Illume_Border_Info* bd_info, E_Illume_Config_Zone *cz)
{
   E_Border* bd;
   E_Border *app;
   int iy, ny, nh;
        ...
        while ((bd = e_container_border_list_prev(bl)))
          {
             ...
             if ((bd->visible) &&
                 ...
                 (((SIZE_EQUAL_TO_ZONE(bd, zone)) &&
                  ...
                  ((bd->client.e.state.ly.curr_ly) &&
                   (E_INTERSECTS(bd->x, bd->y, bd->w, bd->h, zone->x, zone->y, zone->w, zone->h))))) // or split mode check
               {
                  found = EINA_TRUE;
                  break;
               }
        ...
        if ((e_illume_border_is_notification(target_bd) ||
            ...
            && (target_bd_info->indicator_state == E_ILLUME_INDICATOR_STATE_NONE))
          {
                  ...
                  while ((find_bd = e_container_border_list_prev(bl)))
                    {
                       ...
                       if ((find_bd->visible) &&
                           ...
                           (((SIZE_EQUAL_TO_ZONE(find_bd, zone)) &&
                            ...
                            ((find_bd->client.e.state.ly.curr_ly) &&
                             (E_INTERSECTS(find_bd->x, find_bd->y, find_bd->w, find_bd->h, zone->x, zone->y, zone->w, zone->h))))) // or split mode check
                         {
             ...
             if (active_indi_win_found)
               {
                 _policy_property_active_indicator_win_set(find_bd->client.win);
               }
          }
        else
          {
            _policy_property_active_indicator_win_set(target_win);
          }
     }
}
                  ...
                  if (xwin_info->bd_info)
                    {
                       ...
                       if (e_illume_border_is_keyboard(bd)) continue;
                       if (bd->client.illume.win_state.state != ECORE_X_ILLUME_WINDOW_STATE_NORMAL) continue;
   ...
   EINA_LIST_FOREACH(bd->transients, l, child)
     {
        ...
        child_xwin_info = policy_xwin_info_find(child->win);
        if (child_xwin_info)
          {
...
static Eina_Bool
_policy_border_is_sibling(E_Border *bd, E_Border *sibling_bd)
{
   E_Border *parent_bd;
   E_Border *child_bd;
   Eina_List *l = NULL;
   ...
   ret = EINA_FALSE;
   if (!bd || !sibling_bd) return ret;

   parent_bd = bd->parent;
   if (parent_bd)
     {
        EINA_LIST_FOREACH(parent_bd->transients, l, child_bd)
          {
             if (child_bd == sibling_bd)
               {
                  ret = EINA_TRUE;
                  break;
               }
...
_policy_calculate_visibility(void)
{
   ...
   Eina_Bool is_descendent = EINA_FALSE;
   Eina_Bool is_sibling = EINA_FALSE;
   ...
   if (g_block_visibility_calc)
     {
        L(LT_VISIBILITY, "[ILLUME2][VISIBILITY] %s(%d).. Block calculate visibility.. Return!\n",  __func__, __LINE__);
        return;
     }
   ...
   EINA_INLIST_REVERSE_FOREACH (xwin_info_list, xwin_info)
     {
        L(LT_VISIBILITY_DETAIL, "[ILLUME2][VISIBILITY] %s(%d).. win:0x%07x input_only=%d view=%d iconic=%d by_wm=%d is_drawed=%d visi=%d\n",
        __func__, __LINE__, xwin_info->id, xwin_info->attr.input_only, xwin_info->viewable, xwin_info->iconic, xwin_info->iconify_by_wm, xwin_info->is_drawed, xwin_info->visibility);
        // skip "input only" window
        if (xwin_info->attr.input_only) goto check_done;

        // skip "unmap" window
        if ((xwin_info->viewable == 0) &&
            (xwin_info->iconify_by_wm == 0)) goto check_done;
        ...
        is_descendent = EINA_FALSE;
        is_sibling = EINA_FALSE;
        ...
        if ((bd) && (!E_ILLUME_BORDER_IS_IN_MOBILE(bd))) goto check_done;

        if (xwin_info->iconic && (!xwin_info->iconify_by_wm))
          {
             if (bd) goto check_above;
             else goto check_done;
          }
                            ...
                            if (alpha_opaque)
                              {
                                 ...
                                 alpha_opaque_rect.y = xwin_info->attr.y;
                                 alpha_opaque_rect.width = xwin_info->attr.w;
                                 alpha_opaque_rect.height = xwin_info->attr.h;

                                 if (alpha_opaque_region == NULL)
        ...
        if (!bd) goto check_done;

        bd->visibility = xwin_info->visibility;

        // decide if it's the border that DO NOT iconify.
        if (obscured_by_alpha_opaque)
          {
             // Get window region and if there's intersect region with alpha_opaque_region, then it should not iconify!
             Ecore_X_XRegion *dst_region;
             win_region = ecore_x_xregion_new();
             win_rect.x = xwin_info->attr.x;
             win_rect.y = xwin_info->attr.y;
             win_rect.width = xwin_info->attr.w;
             ...
             dst_region = ecore_x_xregion_new();
             ...
             if (win_region)
               {
                  ...
                  if ((dst_region) &&
                      (ecore_x_xregion_intersect(dst_region, win_region, alpha_opaque_region)))
                    {
                       do_not_iconify = EINA_TRUE;
                       xwin_info->do_not_iconify_by_alpha_opaque = EINA_TRUE;
                       ...
                       ecore_x_xregion_free(dst_region);
                    }
                  ecore_x_xregion_free(win_region);
               }
          }
        ...
        if (old_vis != xwin_info->visibility)
          {
             SLOGI("[WM] SEND VISIBILITY. win:0x%07x (old:%d -> new:%d)", xwin_info->bd_info->border->client.win, old_vis, xwin_info->visibility);
                       ...
                       if ((!bd->iconic) && (!do_not_iconify))
                         {
                            ...
                            is_descendent = _policy_border_is_decendent(bd, above_bd);
                            is_sibling = _policy_border_is_sibling(bd, above_bd);

                            if (is_descendent == EINA_FALSE && is_sibling == EINA_FALSE)
                              {
                            ...
                            else if ((is_descendent == EINA_TRUE && above_bd->iconic) || (is_sibling == EINA_TRUE && above_bd->iconic))
                              {
                                 L(LT_ICONIFY, "[ILLUME2][ICONIFY] %s(%d).. Iconify by illume.. win:0x%07x (parent:0x%07x)\n", __func__, __LINE__, xwin_info->bd_info->border->client.win, xwin_info->bd_info->border->parent ? xwin_info->bd_info->border->parent->client.win:(unsigned int)NULL);
                                 _policy_border_iconify_by_illume(xwin_info);
                              }
                  ...
                  if (_e_illume_cfg->use_force_iconify)
                    {
                                 ...
                                 if (above_xwin_info && above_xwin_info->visibility == E_ILLUME_VISIBILITY_FULLY_OBSCURED)
                                   {
                                      ...
                                      if (_policy_border_is_decendent(above_bd, bd))
                                        {
                                           if (above_xwin_info->iconify_by_wm == 1)
                                             {
                       ...
                       /* if window is not iconifyed due to the obscured alpha window, then next time it should be iconifyed. */
                       else if (!obscured_by_alpha_opaque && xwin_info->do_not_iconify_by_alpha_opaque)
                         {
                            if (!bd->iconic && !do_not_iconify && !xwin_info->skip_iconify)
                              {
                                 L(LT_ICONIFY, "[ILLUME2][ICONIFY] %s(%d).. Iconify by obscured_by_alpha_opaque.. win:0x%07x\n", __func__, __LINE__, xwin_info->bd_info->border->client.win);
                                 _policy_border_iconify_by_illume(xwin_info);
                              }
                              xwin_info->do_not_iconify_by_alpha_opaque = EINA_FALSE;
                         }
                       else
                         {
                            if (!xwin_info->vis_changed && !bd->iconic && !do_not_iconify && !xwin_info->skip_iconify)
                              {
                                 L(LT_ICONIFY, "[ILLUME2][ICONIFY] %s(%d).. Iconify because currently uniconified.. win:0x%07x\n", __func__, __LINE__, xwin_info->bd_info->border->client.win);
                                 _policy_border_iconify_by_illume(xwin_info);
                              }
                         }
                    }
               }
             else if (xwin_info->visibility == E_ILLUME_VISIBILITY_UNOBSCURED)
               {
                  L (LT_ANGLE, "[ILLUME2][ANGLE] %s(%d).. visibility is not changed but it has unobscured state.!!! win:0x%07x\n", __func__, __LINE__, xwin_info->bd_info->border->client.win);
                  if (_e_illume_cfg->use_force_iconify)
                    {
                       if (bd->iconic)
                         {
        ...
        if (xwin_info->visibility == E_ILLUME_VISIBILITY_FULLY_OBSCURED)
          {
             if (obscured_by_alpha_opaque && is_opaque_win && (!xwin_info->argb))
               {
                  if (alpha_opaque_region)
                    {
                       win_rect.x = xwin_info->attr.x;
...
check_done:
        xwin_info->vis_changed = EINA_FALSE;
     }
   ...
   e_msg_send("illume.message", "update.visibility", // name + info
               0, // val
               NULL, // obj
               NULL, // msgdata
...
EINTERN E_Illume_XWin_Info*
policy_xwin_info_find(Ecore_X_Window win)
{
...
_policy_xwin_info_add (Ecore_X_Window win)
{
   ...
   E_Illume_XWin_Info* xwin_info = policy_xwin_info_find(win);
   if (xwin_info)
     {
...
void _policy_window_show (Ecore_X_Event_Window_Show *event)
{
   ...
   E_Illume_XWin_Info* xwin_info = policy_xwin_info_find(event->win);
   if (xwin_info == NULL)
     {
        L (LT_XWIN, "[ILLUME2][XWIN] %s(%d).. No win:0x%07x in the list...\n", __func__, __LINE__, event->win);
        return ;
...
void _policy_window_configure (Ecore_X_Event_Window_Configure *event)
{
   ...
   xwin_info = policy_xwin_info_find(event->win);
   if (xwin_info == NULL)
     {
        L (LT_XWIN, "[ILLUME2][XWIN] %s(%d).. No win:0x%07x in the list...\n", __func__, __LINE__, event->win);
        return;
             ...
             if (bd && bd->client.netwm.sync.counter)
               {
                  changed_size = 1;
                  check_visibility |= 2;
                  xwin_info->wait_resize = EINA_FALSE;
   ...
   if (!changed_size)
     ...
     {
        if (bd && bd->client.e.state.ly.support)
          {
             if (bd->client.e.state.ly.curr_ly ||
                 bd->client.e.state.ly.prev_ly)
               {
                  xwin_info->attr.w = event->w;
                  xwin_info->attr.h = event->h;
               }
        ...
        if (old_above_xwin_info)
          {
             ...
             for (; temp_xwin_info; temp_xwin_info = (EINA_INLIST_GET(temp_xwin_info)->prev ? _EINA_INLIST_CONTAINER(temp_xwin_info, EINA_INLIST_GET(temp_xwin_info)->prev) : NULL))
               {
                  if (!temp_xwin_info->attr.input_only && !temp_xwin_info->viewable)
                    {
                       old_above_xwin_info = temp_xwin_info;
                       continue;
                    }
               }
          }
   ...
   new_above_xwin_info = policy_xwin_info_find(event->abovewin);
                  ...
                  if (changed_size)
                    {
                       if ((target_xwin_info == xwin_info) &&
                         ...
                         {
                            L(LT_VISIBILITY, "[ILLUME2][VISIBILITY] %s(%d).. SIZE is changed... target win:0x%07x  win:%x type:%d\n",  __func__, __LINE__, target_xwin_info->id, xwin_info->id, check_visibility);
                            target_xwin_info->wait_draw = EINA_TRUE;
                            return;
                         }
                    }
                  L(LT_VISIBILITY, "[ILLUME2][VISIBILITY] %s(%d).. visibility is changed... win:0x%07x type:%d\n",  __func__, __LINE__, event->win, check_visibility);
                  _g_visibility_changed = EINA_TRUE;
                  target_xwin_info->vis_changed = EINA_TRUE;
               }
...
void _policy_window_configure_request (Ecore_X_Event_Window_Configure_Request *event)
{
   ...
   policy_assistant_menu_configure_request(e);
                  ...
                  if (e->detail == ECORE_X_WINDOW_STACK_BELOW && !e->abovewin)
                    {
                       ...
                       if (xwin_info)
                         {
                            // reset above_xwin for calculating visibility
                            xwin_info->above_xwin = bd->client.win;
...
void _policy_window_sync_draw_done(Ecore_X_Event_Client_Message* event)
{
   ...
   xwin_info = policy_xwin_info_find(bd->win);
   if (!xwin_info) return;
   if (xwin_info->wait_resize) return;
...
void _policy_illume_win_state_change_request (Ecore_X_Event_Client_Message *event)
{
   ...
   else if (atom == ECORE_X_ATOM_E_ILLUME_WINDOW_STATE_ASSISTANT_MENU)
     {
...
void _policy_illume_internal_client_message(Ecore_X_Event_Client_Message *event)
{
   ...
   else if (event->message_type == E_ILLUME_SPLIT_LAUNCHER_WINDOW_SHOW)
     {
        if (!e_illume_border_is_split_launcher(bd)) return;
     }
...
static void
_calculate_delta_pos_from_knob(E_Illume_Border_Info *bd_info, int ev_x, int ev_y, int direction)
{
   /* calculate the distance between knob(resize_handler) and
      x,y positon from button_press event. */
   E_Border *bd;
         ...
         switch (direction)
           {
            ...
            case ECORE_X_NETWM_DIRECTION_SIZE_TL:
            case ECORE_X_NETWM_DIRECTION_SIZE_TR:
               bd_info->resize_req.mouse.knob_dx = ev_x - bd->x;
               break;

            case ECORE_X_NETWM_DIRECTION_SIZE_B:
            case ECORE_X_NETWM_DIRECTION_SIZE_BL:
            case ECORE_X_NETWM_DIRECTION_SIZE_BR:
               bd_info->resize_req.mouse.knob_dx = bd->x + bd->w - ev_x;
               break;
            default:
               bd_info->resize_req.mouse.knob_dx = 0;
               break;
           }
         ...
         switch (direction)
           {
            ...
            case ECORE_X_NETWM_DIRECTION_SIZE_TR:
            case ECORE_X_NETWM_DIRECTION_SIZE_BR:
               bd_info->resize_req.mouse.knob_dy = ev_y - bd->y;
               break;

            case ECORE_X_NETWM_DIRECTION_SIZE_L:
            case ECORE_X_NETWM_DIRECTION_SIZE_TL:
            case ECORE_X_NETWM_DIRECTION_SIZE_BL:
               bd_info->resize_req.mouse.knob_dy = bd->y + bd->h - ev_y;

               break;
            default:
               bd_info->resize_req.mouse.knob_dy = 0;
               break;
           }
         break;
...
void _policy_window_move_resize_request(Ecore_X_Event_Window_Move_Resize_Request *event)
{
   ...
   _calculate_delta_pos_from_knob(bd_info, e->x, e->y, e->direction);
        ...
        switch (e->state[i])
          {
           ...
           default:
             ...
             policy_assistant_menu_window_state_update(bd, e->state[i], e->action);
             break;
          }
     }
   ...
   if (!strncmp(info, "visibility.src", sizeof("visibility.src")))
     {
        ...
        if (bd) _policy_active_rotation_bd_update(bd->zone);

        xwin_info = policy_xwin_info_find(win);
        if (!xwin_info) return;
             ...
             if (!bd->iconify_resize.set)
               {
                  ...
                  ELBF(ELBT_BD, 0, bd->client.win,"Force Iconify-Resize(bd): Geo (%d, %d, %d, %d)", bd->x, bd->y, bd->w, bd->h);
                  e_border_force_render_request(bd,EINA_FALSE);
               }
          }
     }
             ...
             if (bd->iconify_resize.set)
               {
                  ...
                  ELBF(ELBT_BD, 0, bd->client.win,"Force UnIconify-Resize(bd): Geo (%d, %d, %d, %d)", bd->x, bd->y, bd->w, bd->h);
                  e_border_force_render_request(bd,EINA_FALSE);
               }
          }
     }
                       ...
                       if (!e_object_is_del(E_OBJECT(child)))
                         {
                            if (child->iconic)
                              {
                                 xwin_info = policy_xwin_info_find(child->win);
        ...
        EINA_LIST_REVERSE_FOREACH(bd->zone->container->layers[i].clients, l, b)
          {
             ...
             if (ecore_x_xregion_is_empty(visible_region))
               {
                  ecore_x_xregion_free(visible_region);
                  return below_list;
               }
          }
     }
                  ...
                  while (parent_bd)
                    {
                       ...
                       if (parent_bd->iconic)
                         {
                            parent_xwin_info = policy_xwin_info_find(parent_bd->win);
...
_policy_window_desk_set(Ecore_X_Event_Client_Message *event)
{
   ...
   if ((bd = win))
     {
        e_border_find_by_client_window(win);
     }
   else if (bl)
     {
...
/* for supporting rotation */
EINTERN void
policy_border_rotation_type_update(E_Border *bd)
{
   int old_type, type = E_BORDER_ROTATION_TYPE_NORMAL;

   EINA_SAFETY_ON_NULL_RETURN(bd);

   old_type = bd->client.e.state.rot.type;
   if ((bd->client.netwm.type == ECORE_X_WINDOW_TYPE_NORMAL) &&
       (bd->client.e.state.ly.curr_ly))
     type = E_BORDER_ROTATION_TYPE_DESK_LAYOUT;
   else if (policy_border_is_rot_dependent(bd))
     type = E_BORDER_ROTATION_TYPE_DEPENDENT;

#define ROT_TYPE_TO_STRING(type)                                     \
   (type == E_BORDER_ROTATION_TYPE_NORMAL) ? "NORMAL" :              \
   (type == E_BORDER_ROTATION_TYPE_DEPENDENT) ? "DEPENDENT" : "DESK"
   ELBF(ELBT_ROT, 0, bd->client.win, "UPDATE ROT_TYPE: %s -> %s",
        ROT_TYPE_TO_STRING(old_type), ROT_TYPE_TO_STRING(type));
#undef ROT_TYPE_TO_STRING

   bd->client.e.state.rot.type = type;
   if (old_type != type) bd->client.e.fetch.rot.need_rotation = EINA_TRUE;
}
   ...
   if (bd->internal)
     {
        ...
        policy_assistant_menu_icon_rotation(bd, rotation);
     }
...
/* find and update the active rotation border.
 * we will call the border(which is for dependent rotation reference) active rotation border.
 */
static void
_policy_active_rotation_bd_update(E_Zone *zone)
{
   E_Border_List *bl;
   E_Border *bd;
   E_Illume_Border_Info *bd_info = NULL;
        ...
        while ((bd = e_container_border_list_prev(bl)))
          {
             ...
             if (e_object_is_del(E_OBJECT(bd))) continue;
             // dependent rotation bd isn't to be active rotation bd
             if ((policy_border_is_rot_dependent(bd)) &&
                 (!e_illume_border_is_floating(bd))) continue;

             if ((SIZE_EQUAL_TO_ZONE(bd, zone)) ||  // full size check
                 ((bd->client.e.state.ly.curr_ly) &&
                  (E_INTERSECTS(bd->x, bd->y, bd->w, bd->h, zone->x, zone->y, zone->w, zone->h)))) // or split mode check
               {
                  bd_info = policy_get_border_info(bd);
                  if ((!bd->client.argb) ||
                      ((bd_info) && (bd_info->opaque)))
                    {
                       if (dep_rot.active_bd != bd)
                         {
                            ...
                            if (!e_illume_border_is_notification(bd) ||
                              ...
                              {
                                 ELBF(ELBT_ROT, 0, bd->client.win,
                                      "ROT ACTIVE WIN: %s(0x%08x) -> %s(0x%08x)",
                                      dep_rot.active_bd ? dep_rot.active_bd->client.icccm.name : "NULL",
                                      dep_rot.active_bd ? dep_rot.active_bd->client.win : (unsigned int)NULL,
                                      bd->client.icccm.name, bd->client.win);

                                 SLOGI("ROT ACTIVE WIN: %s(0x%08x) -> %s(0x%08x)",
                                       dep_rot.active_bd ? dep_rot.active_bd->client.icccm.name : "NULL",
                                       dep_rot.active_bd ? dep_rot.active_bd->client.win : (unsigned int)NULL,
                                       bd->client.icccm.name, bd->client.win);
                                 dep_rot.active_bd = bd;
                                 rot = e_border_rotation_is_progress(bd) ?
                                    e_border_rotation_next_angle_get(bd) :
                                    e_border_rotation_curr_angle_get(bd);
                                 _policy_border_dep_rotation_all_set(rot);
                              }
                            e_zone_rotation_sub_set(bd->zone, rot);
                         }
                       found = EINA_TRUE;
...
_policy_idle_enterer(void)
{
   ...
   policy_assistant_menu_idle_enterer();
}
...
#define FLOATING_PORT_MARGIN_TOP 30
   ...
   switch (angle)
     {
      case 0:
         ...
         max_w = bd->zone->w;
         max_h = bd->zone->h - kbd_h - FLOATING_PORT_MARGIN_TOP;
         ...
         if (bd->h > max_h)
              ...
              if (max_h >= min_h)
                {
                   ...
                   *w = bd->w;
                   ...
                   gap = FLOATING_PORT_MARGIN_TOP - bd->y;
                   if (gap > 0)
                     {
                        *y = bd->y;
                        *h = max_h + gap;
         ...
         else
           {
              ...
              *x = bd->x;
              *y = bd->zone->h - kbd_h - bd->h;
           }
         break;

      case 90:
         // 4. check bd's height is bigger than available height(= zone's h - kbd's h - top margin - bottom margin)
         max_w = bd->zone->w - kbd_w;
         max_h = kbd_h;
         ELBF(ELBT_ILLUME, 0, bd->client.win, "Geo (%d, %d, %d, %d) is partially obscured by KEYBOARD. max(%d, %d)", bd->x, bd->y, bd->w, bd->h, max_w, max_h);
         if (bd->w > max_w)
           {
              ...
              if (max_h >= min_h)
                {
                   ...
                   *x = bd->x;
                   *w = bd->w;
                   gap = FLOATING_PORT_MARGIN_TOP - (bd->zone->h - bd->y - bd->h);
                   if (gap > 0)
                     {
                        *y = kbd_h;
                        *h = max_h + gap;
              ...
              else
                {
                   ...
                   *w = bd->w;
                   ...
                   if (gap > 0)
                     {
                        *y = bd->zone->h - min_h - FLOATING_PORT_MARGIN_TOP;
                        *h = min_h + gap;
                     }
                   else
                     {
...
static void
_policy_check_modal_win_clipboard_policy(E_Border *bd, int *x, int *y, int *w, int *h)
{
   int angle;
   int new_x, new_y, new_w, new_h;
   int cb_w, cb_h;
   ...
   res = ecore_x_e_illume_clipboard_geometry_get(bd->client.win, 0, 0, &cb_w, &cb_h);
   if (res)
     {
        state = ecore_x_e_illume_clipboard_state_get(bd->client.win);
        if (state == ECORE_X_ILLUME_CLIPBOARD_STATE_ON)
          {
             // popup placed to the cender of area(except clipboard)
             switch (angle)
               {
                case 0:
                   target_w = bd->zone->w - cb_w;
                   target_h = bd->zone->h - cb_h;
                   new_y = (bd->zone->y + ((target_h - new_h) / 2));
                   break;

                case 90:
                   target_w = bd->zone->w - cb_h;
                   target_h = bd->zone->h - cb_w;
                ...
                case 180:
                   target_w = bd->zone->w - cb_w;
                   target_h = bd->zone->h - cb_h;
                   new_y = (bd->zone->y + cb_w + ((target_h - new_h) / 2));
                   break;

                case 270:
                   target_w = bd->zone->w - cb_h;
                   target_h = bd->zone->h - cb_w;
                   new_x = (bd->zone->x + cb_h + ((target_w - new_w) / 2));
                   break;
...
_policy_check_floating_win_keyboard_policy(E_Border *bd)
{
   ...
   if (change_size ||
       !bd_info->floating_size.rot.remember[angle_id])
     {
...
_policy_property_keyboard_geometry_change(Ecore_X_Event_Window_Property *event)
{
   ...
   if (bd->client.illume.win_state.state != ECORE_X_ILLUME_WINDOW_STATE_NORMAL)
   ...
   else
     {
        ...
        kbd_bd = e_illume_border_keyboard_get(bd->zone);
        if (kbd_bd)
           policy_assistant_menu_icon_relayout_by_keyboard(kbd_bd);
...
static Eina_Bool
_policy_property_force_render_done(void *data __UNUSED__, int type __UNUSED__, void *event)
{
   E_Event_Border_Force_Render_Done *e = event;
   E_CHECK_RETURN(e, EINA_FALSE);
   E_Border *bd = e->border;
   E_CHECK_RETURN(bd, EINA_FALSE);
   E_Illume_Border_Info *bd_info = policy_get_border_info(bd);
   E_CHECK_RETURN(bd_info, EINA_FALSE);

   //send to appcore recapture window
   ecore_x_client_message32_send(bd->client.win,
                                 E_ILLUME_ATOM_APPCORE_RECAPTURE_REQUEST,
                                 ECORE_X_EVENT_MASK_WINDOW_CONFIGURE,
                                 0,0,0,0,0);

   SLOGI("[WM] SEND RECAPTURE REQUEST TO APPCORE. win:0x%07x", bd->client.win);
   if (bd_info->force_render_handler)
     {
        ecore_event_handler_del(bd_info->force_render_handler);
        bd_info->force_render_handler = NULL;
     }
...
static void
_policy_property_keyboard_state_change(Ecore_X_Event_Window_Property *event)
{
   E_Border *bd;
   E_Illume_Border_Info *bd_info;
   ...
   if (!ecore_x_window_prop_atom_get(bd->client.win, ECORE_X_ATOM_E_VIRTUAL_KEYBOARD_STATE,&atom, 1))
      return;

#if 0
   /* This codes are useful when Task manager requires App's Preview Image*/
   if ((bd->visibility == E_ILLUME_VISIBILITY_FULLY_OBSCURED) &&
       (atom == ECORE_X_ATOM_E_VIRTUAL_KEYBOARD_OFF) &&
       (bd->client.icccm.state == ECORE_X_WINDOW_STATE_HINT_ICONIC))
     {
        if (!bd_info->force_render_handler)
          {
             e_border_force_render_request(bd,EINA_FALSE);
             bd_info->force_render_handler = ecore_event_handler_add(E_EVENT_BORDER_FORCE_RENDER_DONE,_policy_property_force_render_done, NULL);
          }
     }
#endif
}
...
static void
_policy_property_window_aux_hint_startup_get(E_Border *bd)
{
   int ret;
   int count;
   unsigned char* prop_data = NULL;
   ...
   ret = ecore_x_window_prop_property_get(bd->client.win, E_WINDOW_AUX_HINT_STARTUP, ECORE_X_ATOM_CARDINAL, 32, &prop_data, &count);
   if (ret && prop_data)
     {
        if (count == 2)
          {
             bd->launch_info.launch_by = ((int *)prop_data)[0];
             bd->launch_info.launch_to = ((int *)prop_data)[1];
          }
     }
...
static void
_policy_property_window_aux_hint_startup_change(Ecore_X_Event_Window_Property *event)
{
   E_Border* bd;
   ...
   if (g_screen_lock_info->is_lock)
     {
        ...
        L(LT_SCREEN_LOCK, "[ILLUME2][LOCK] %s(%d)... De-activate BLOCK SCREEN by illume...\n", __func__, __LINE__);
     }

   policy_border_visibility_calc_block(EINA_FALSE);
   _g_visibility_changed = EINA_TRUE;
...
{
   if (!_e_illume_cfg->use_screen_lock) return;
   if (!man) return;
...
EINTERN void
policy_border_add_block_list(E_Border* bd, Eina_Bool screen_lock)
{
   if (!_e_illume_cfg->use_screen_lock) return;
   ...
   if (g_screen_lock_info->is_lock == EINA_FALSE)
     {
        L(LT_SCREEN_LOCK, "[ILLUME2][LOCK] %s(%d)... Activate BLOCK SCREEN by illume...\n", __func__, __LINE__);
        g_screen_lock_info->man = bd->zone->container->manager;
        g_screen_lock_info->is_lock = EINA_TRUE;
     }
   else
     {
   ...
   // block screen update
   if (screen_lock == EINA_TRUE)
     {
        if (g_screen_lock_info->is_block_scr_update == EINA_FALSE)
          {
             g_screen_lock_info->is_block_scr_update = EINA_TRUE;
   ...
   if (!g_screen_lock_info->lock_timer)
     {
        g_screen_lock_info->lock_timer = ecore_timer_add(_e_illume_cfg->timeout_screen_lock,
                                                         _policy_screen_lock_timeout,
                                                         g_screen_lock_info->man);
     }

   // block calculate visibility
   policy_border_visibility_calc_block(EINA_TRUE);
...
EINTERN void
policy_border_remove_block_list(E_Border* bd)
{
   if (!_e_illume_cfg->use_screen_lock) return;
...
EINTERN void
policy_border_visibility_calc_block(Eina_Bool is_block)
{
   L(LT_VISIBILITY, "[ILLUME2][VISIBILITY] %s(%d).. calc visibility BLOCK:%d\n",  __func__, __LINE__, is_block);

   if (!_e_illume_cfg->use_screen_lock)
      g_block_visibility_calc = is_block;
   else
     {
        if (is_block == EINA_FALSE)
          {
             if (!g_screen_lock_info->blocked_list)
               {
                  g_block_visibility_calc = is_block;
               }
             else
               {
                  L(LT_VISIBILITY, "[ILLUME2][VISIBILITY] %s(%d).. Currenlty SCREEN_LOCKED.. So Ignore request.\n",  __func__, __LINE__);
               }
          }
        else
          {
