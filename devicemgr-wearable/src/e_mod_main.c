...
_e_devicemgr_init(void)
{
   ...
   e_devicemgr.touch_device_id = 0;
   e_devicemgr.touch_rotation_node = -1;
   ...
   e_devicemgr.atomTouchRotationAngle = ecore_x_atom_get(EVDEVMULTITOUCH_PROP_ROTATION_ANGLE);
   e_devicemgr.atomTouchRotationNode = ecore_x_atom_get(EVDEVMULTITOUCH_PROP_ROTATION_NODE);
   e_devicemgr.atomTouchSwapAxes = ecore_x_atom_get(EVDEVMULTITOUCH_PROP_SWAP_AXES);
   e_devicemgr.atomTouchInvertAxes = ecore_x_atom_get(EVDEVMULTITOUCH_PROP_INVERT_AXES);
   e_devicemgr.atomGestureDeviceId = ecore_x_atom_get(GESTURE_DEVICE_ID);
   ...
   SLOG(LOG_DEBUG, "DEVICEMGR", "[e_devicemgr] e_devicemgr.atomTouchRotationAngle = %d\n", e_devicemgr.atomTouchRotationAngle);
   SLOG(LOG_DEBUG, "DEVICEMGR", "[e_devicemgr] e_devicemgr.atomTouchRotationNode = %d\n", e_devicemgr.atomTouchRotationNode);
   SLOG(LOG_DEBUG, "DEVICEMGR", "[e_devicemgr] e_devicemgr.atomTouchSwapAxes = %d\n", e_devicemgr.atomTouchSwapAxes);
   SLOG(LOG_DEBUG, "DEVICEMGR", "[e_devicemgr] e_devicemgr.atomTouchInvertAxes = %d\n", e_devicemgr.atomTouchInvertAxes);
   SLOG(LOG_DEBUG, "DEVICEMGR", "[e_devicemgr] e_devicemgr.atomGestureDeviceId = %d\n", e_devicemgr.atomGestureDeviceId);
   ...
   if(EINA_FALSE == e_mod_sf_rotation_init(e_devicemgr.touch_device_id))
     {
   ...
   for (i = 0; i < ndevices ; i++)
     {
        ...
        if (strcasestr(dev->name, "Gesture"))
          {
             ecore_x_window_prop_card32_set(e_devicemgr.rootWin, e_devicemgr.atomGestureDeviceId, &dev->deviceid, 1);
             SLOG(LOG_DEBUG, "DEVICEMGR", "[e_devicemgr] gesture device id=%d\n", dev->deviceid);
             continue;
	   }
        ...
        switch (dev->use)
          {
             case XISlavePointer:
                ...
                if (1==_e_devicemgr_check_device_type(dev->deviceid, E_DEVICEMGR_TOUCHSCREEN, dev->name))
                  {
                     ...
                     e_devicemgr.touch_device_id = dev->deviceid;
                     _e_devicemgr_get_touch_rotation_node(e_devicemgr.touch_device_id);
                ...
                else if (data->name && (strcasestr(data->name, "tizen_rotary")))
                  {
                     data->type = E_DEVICEMGR_MOUSE;
                     e_devicemgr.device_list = eina_list_append(e_devicemgr.device_list, data);
                     e_devicemgr.num_pointer_devices++;
                     _e_devicemgr_detach_slave(data->id);
                     SLOG(LOG_DEBUG, "DEVICEMGR",  "[e_devicemgr][%s] Rotary device (id=%d, name=%s, num_touchscreen_devices=%d) was added/enabled !\n",
                                 __FUNCTION__, data->id, info->name, e_devicemgr.num_pointer_devices);
                  }
                else if (data->name && (strcasestr(data->name, "tizen_detent")))
                  {
                     data->type = E_DEVICEMGR_MOUSE;
                     e_devicemgr.device_list = eina_list_append(e_devicemgr.device_list, data);
                     e_devicemgr.num_pointer_devices++;
                     _e_devicemgr_detach_slave(data->id);
                     SLOG(LOG_DEBUG, "DEVICEMGR",  "[e_devicemgr][%s] hall device (id=%d, name=%s, num_touchscreen_devices=%d) was added/enabled !\n",
                                 __FUNCTION__, data->id, info->name, e_devicemgr.num_pointer_devices);
                  }
                else
                  {
...
static void
_e_devicemgr_get_touch_rotation_node(int deviceid)
{
   Atom act_type;
   unsigned long nitems, bytes_after;
   ...
   if (XIGetProperty(e_devicemgr.disp, deviceid, e_devicemgr.atomTouchRotationNode, 0, 1000, False,
                              XA_INTEGER, &act_type, &act_format,
                              &nitems, &bytes_after, &data) != Success)
     {
        SLOG(LOG_DEBUG, "DEVICEMGR",  "[e_devicemgr][get_touch_rotation_info] Failed to get XI2 device property !(deviceid=%d)\n", deviceid);
        goto out;
     }
        ...
        switch(act_type)
          {
                case XA_INTEGER:
                    ...
                    {
                        case 32:
                            e_devicemgr.touch_rotation_node = *((int32_t*)ptr);
                            SLOG(LOG_DEBUG, "DEVICEMGR",  "[e_devicemgr][get_touch_rotation_info] touch_rotation_node=%d\n", e_devicemgr.touch_rotation_node);
...
void
_e_devicemgr_set_touch_rotation_angle(int deviceid, int angle)
{
   if (e_devicemgr.touch_rotation_node > 0)
     {
        XIChangeProperty(e_devicemgr.disp, deviceid, e_devicemgr.atomTouchRotationAngle, XA_INTEGER, 8,
                              PropModeReplace, &angle, 1);
   ...
   else
     {
        ...
        SLOG(LOG_DEBUG, "DEVICEMGR",  "[e_devicemgr] SwapAxes=%d, InvertX=%d, InvertY=%d\n", swap, invert[0], invert[1]);

        XIChangeProperty(e_devicemgr.disp, deviceid, e_devicemgr.atomTouchSwapAxes, XA_INTEGER, 8,
                              PropModeReplace, &swap, 1);
        XIChangeProperty(e_devicemgr.disp, deviceid, e_devicemgr.atomTouchInvertAxes, XA_INTEGER, 8,
                              PropModeReplace, invert, 2);
        XFlush(e_devicemgr.disp);
        XSync(e_devicemgr.disp, False);
     }
}
   ...
   switch(info->use)
     {
        case XISlavePointer:
           ...
           if (1==_e_devicemgr_check_device_type(id, E_DEVICEMGR_TOUCHSCREEN, info->name))
             {
                //Now we have a touchscreen device.
                e_devicemgr.touch_device_id = id;
                _e_devicemgr_get_touch_rotation_node(e_devicemgr.touch_device_id);
                data->type = E_DEVICEMGR_TOUCHSCREEN;
           ...
           if (strcasestr(info->name, "keyboard"))//keyboard
             {
                ...
                int res = ecore_x_window_prop_card32_get(e_devicemgr.rootWin, e_devicemgr.atomExKeyboardEnabled, &ret_val, 1);
                if(res <= 0)
