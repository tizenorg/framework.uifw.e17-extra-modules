   ...
   if (cw->counter)
     {
        if (cw->bd)
          {
             ...
             if (client_cw &&
                 client_cw->counter == cw->counter)
               {
                  ...
                  L(LT_EVENT_X,
                    "COMP|%31.31s|w:0x%08x|done:%d val:%d SYNC_SETUP\n",
                    "INC", e_mod_comp_util_client_xid_get(cw),
                    cw->sync_info.done_count, cw->sync_info.val);
                  return;
               }
        ...
        L(LT_EVENT_X,
          "COMP|%31.31s|w:0x%08x|done:%d val:%d SYNC_SETUP\n",
          "BEGIN INC", e_mod_comp_util_client_xid_get(cw),
          cw->sync_info.done_count, cw->sync_info.val);
     }
}
             ...
             if (cw->counter)
               {
                  ...
                  L(LT_EVENT_X,
                    "COMP|%31.31s|w:0x%08x|done:%d val:%d PROP_SYNC\n",
                    "CANCEL INC", e_mod_comp_util_client_xid_get(cw),
                    cw->sync_info.done_count, cw->sync_info.val);
               }
             cw->counter = counter;
             if (cw->counter)
               {
                  ...
                  L(LT_EVENT_X,
                    "COMP|%31.31s|w:0x%08x|done:%d val:%d PROP_SYNC\n",
                    "INC BEGIN", e_mod_comp_util_client_xid_get(cw),
                    cw->sync_info.done_count, cw->sync_info.val);
               }
          }
     }
