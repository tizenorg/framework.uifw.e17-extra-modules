                                   ...
                                   FILE          *fs)
{
   ...
   fprintf(fs, " canvas->nocomp run:%d force_composite:%d\n", canvas->nocomp.run, canvas->nocomp.force_composite);
   fprintf(fs, " canvas->nocomp cw:0x%08x\n", canvas->nocomp.cw ? e_mod_comp_util_client_xid_get(cw) : 0);
   ...
   if (logtype == LT_CREATE)
   ...
   else if (logtype == LT_DRAW)
     {
        ...
        snprintf(buf, sizeof(buf),
                 "This is a test dialog for debugging purpose.<br>"
                 "Rendering mode: %s",
                 c->nocomp ? "No composite" : "Composite");
        e_util_dialog_internal("Enlightenment Rendering Mode", buf);
     }
