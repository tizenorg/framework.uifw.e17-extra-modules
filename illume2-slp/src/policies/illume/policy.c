        ...
        if (e_illume_border_is_lock_screen(active_bd))
          {
             ...
             Eina_Bool passed;
             /* determine layering position */
             pos = _policy_border_layer_map(active_bd->layer);
             passed = EINA_FALSE;
             ...
             for (i = pos; i >= 2; i--)
               {
                  ...
                  EINA_LIST_REVERSE_FOREACH(active_bd->zone->container->layers[i].clients, l, b)
                    {
                       if (!b) continue;
                       ...
                       /* skip if it's not on this zone */
                       if (b->zone != active_bd->zone) continue;
...
static void _policy_property_window_opaque_change (Ecore_X_Event_Window_Property *event)
{
   ...
   L(LT_VISIBILITY, "[ILLUME2][VISIBILITY] %s(%d).. CALL calculate_visibility...\n",  __func__, __LINE__);
...
int _policy_init (void)
{
   ...
   g_screen_lock_info = (E_Illume_Screen_Lock_Info*) calloc (1, sizeof (E_Illume_Screen_Lock_Info));
...
static void _policy_change_indicator_layer(E_Border *indi_bd, E_Border *bd, int layer, int level)
{
   ...
   if (bd)
     ...
     {
        // if bd is null, this means target is not notification window
...
_policy_calculate_visibility (Ecore_X_Window win)
{
   ...
   // 1. calculates window's region until the windos which is placed on the target window
   //
   L (LT_VISIBILITY_DETAIL, "[ILLUME2][VISIBILITY] PASS-----1\n");
   if (xwin_info_list)
     {
        EINA_INLIST_REVERSE_FOREACH (xwin_info_list, xwin_info)
          {
             L (LT_VISIBILITY_DETAIL, "[ILLUME2][VISIBILITY] xwin_info(win:0x%07x, (%d, %d, %d x %d) visibility:%d\n", xwin_info->id, xwin_info->attr.x, xwin_info->attr.y, xwin_info->attr.w, xwin_info->attr.h, xwin_info->visibility);
             is_opaque = 0;
             ...
             if (((!xwin_info->argb) && (!xwin_info->attr.input_only)) ||
                 (is_opaque))
               {
                  ...
                  if (ecore_x_xregion_is_empty(region) == EINA_TRUE)
                    {
                       L (LT_VISIBILITY_DETAIL, "[ILLUME2][VISIBILITY] region is empty... Rest Windows are FULLY OBSCURED...\n");
                       if (_e_illume_cfg->use_force_iconify)
                         {
                            if (xwin_info->bd_info && xwin_info->bd_info->border)
                              {
                                 last_visible_bd = xwin_info->bd_info->border;
                                 L(LT_ICONIFY, "[ILLUME2][ICONIFY] %s(%d)... last visible win:0x%07x\n", __func__, __LINE__, last_visible_bd->client.win);
                              }
                         }
                    }
               }
   ...
   if (xwin_info == NULL)
     {
        L (LT_VISIBILITY_DETAIL, "[ILLUME2][VISIBILITY] BAD.... current border is not in the raise_Stack...\n");
   ...
   //     - when a fully obscured window is found. then all rest windows are fully obscured
   //
   L (LT_VISIBILITY_DETAIL, "[ILLUME2][VISIBILITY] PASS-----2,3\n");
   if (xwin_info_list)
     {
        for (; xwin_info; xwin_info = (EINA_INLIST_GET(xwin_info)->prev ? _EINA_INLIST_CONTAINER(xwin_info, EINA_INLIST_GET(xwin_info)->prev) : NULL))
          {
             ...
             if (is_fully_obscured)
               {
                  L (LT_VISIBILITY_DETAIL, "[ILLUME2][VISIBILITY] Win:0x%07x is FULLY OBSCURED...\n", xwin_info->id);
                       ...
                       if (xwin_info->bd_info && xwin_info->bd_info->border)
                         {
                            ...
                            if (_e_illume_cfg->use_force_iconify)
                              {
                                 L(LT_ICONIFY, "[ILLUME2][ICONIFY] %s(%d)... last visible win:0x%07x\n", __func__, __LINE__, last_visible_bd ? last_visible_bd->client.win : (unsigned int)NULL);
                                 if (!((last_visible_bd) &&
                                       (last_visible_bd->parent == xwin_info->bd_info->border)))
                                   {
        ...
        if (visible)
          {
             ...
             L(LT_VISIBILITY, "[ILLUME2][VISIBILITY] %s(%d).. CALL calculate_visibility...\n",  __func__, __LINE__);
             _policy_calculate_visibility(win);
...
{
   if ((!strncmp(event->name, "comp-slp", sizeof("comp-slp"))) ||
...
void _policy_border_iconify_cb(E_Border *bd)
{
   ...
   L(LT_VISIBILITY, "[ILLUME2] %s(%d).. CALL calculate_visibility...\n",  __func__, __LINE__);
   _policy_calculate_visibility(bd->win);
                  ...
                  if (!child->iconic)
                    {
                       ...
                       _policy_border_force_iconify(child);
                    }
               }
          }
     }
