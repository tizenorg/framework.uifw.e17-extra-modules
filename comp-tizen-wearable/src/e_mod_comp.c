                    ...
                    Ecore_X_Window win)
{
   ...
   if (!ecore_x_window_attributes_get(cw->win, &att))
     {
        SECURE_SLOGD("[CW Creating Fail!!(Attributes get)] Win:%08x", e_mod_comp_util_client_xid_get(cw));
        if (cw->bd)
          {
   ...
   if ((!cw->input_only) && (!cw->invalid))
        ...
        if (!cw->objs)
          {
             ...
             SECURE_SLOGD("[CW Creating Fail!!(Object Add fail)] Win:%08x", e_mod_comp_util_client_xid_get(cw));
             if (cw->bd)
               {
                  eina_hash_del(borders, e_util_winid_str_get(cw->bd->client.win), cw);
                  if (cw->dfn) e_object_delfn_del(E_OBJECT(cw->bd), cw->dfn);
   ...
   else
     {
        ...
        if (!cw->objs)
          {
             SECURE_SLOGD("[CW Creating Fail!!(Input only Object Add fail)] Win:%08x", e_mod_comp_util_client_xid_get(cw));
             if (cw->bd)
               {
                  eina_hash_del(borders, e_util_winid_str_get(cw->bd->client.win), cw);
                  if (cw->dfn) e_object_delfn_del(E_OBJECT(cw->bd), cw->dfn);
...
_e_mod_comp_active_request_handle(Ecore_X_Event_Client_Message *ev)
{
   ...
   if ((cw) && (TYPE_HOME_CHECK(cw)))
     {
        ELBF(ELBT_COMP, 0, e_mod_comp_util_client_xid_get(cw),
        "%15.15s|", "HOME ACTIVE");

        cw->activate = EINA_TRUE;
        cw->c->effect_funcs.win_show(cw);
        cw->activate = EINA_FALSE;
     }
}
...
_e_mod_comp_hib_leave(Ecore_X_Event_Client_Message *ev)
{
   ...
   if (cfg->effect_policy_app_callscreen) eina_stringshare_del(cfg->effect_policy_app_callscreen);
