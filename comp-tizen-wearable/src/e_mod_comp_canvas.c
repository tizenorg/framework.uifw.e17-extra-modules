                      ...
                      E_Zone *zone)
{
   ...
   if (_comp_mod->conf->use_hwc)
     {
        canvas->hwcomp = e_mod_comp_hwcomp_new (canvas);
        if (!canvas->hwcomp)
          {
             e_util_dialog_internal
               (_("Compositor Error"),
...
e_mod_comp_canvas_del(E_Comp_Canvas *canvas)
{
   ...
   if (_comp_mod->conf->use_hwc)
     {
        e_mod_comp_hwcomp_free (canvas->hwcomp);
        canvas->hwcomp = NULL;
     }
   ...
   EINA_LIST_FOREACH(c->canvases, l, canvas)
     {
        ...
        if (_comp_mod->conf->use_hwc)
          {
            if ((canvas->hwcomp) && (canvas->hwcomp->fullcomp_pending))
              canvas->hwcomp->fullcomp_pending--;
          }
