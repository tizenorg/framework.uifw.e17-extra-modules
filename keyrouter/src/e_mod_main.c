...
static Eina_Bool LongPressEventDeliver(void *data)
{
	...
	if(XKeycodeToKeysym(keyrouter.disp, lpinfo->keycode, col) == XKeycodeToKeysym(keyrouter.disp, menu_keycode, col))
	{
	...
	else if(XKeycodeToKeysym(keyrouter.disp, lpinfo->keycode, col) == XKeycodeToKeysym(keyrouter.disp, back_keycode, col))
	{
...
_e_keyrouter_hwkey_event_handler(XEvent *ev)
{
	...
	{
		if(ev->xkey.keycode == XKeysymToKeycode(keyrouter.disp, XStringToKeysym(KEY_VOICEWAKEUP)))
		{
			SLOG(LOG_DEBUG, "HIGEAR", "[36mVoice Wakeup Key Released![0m\n");
		}
	...
	else if( ev->type == KeyPress )	//KeyPress handling for key composition
	{
		...
		if(ev->xkey.keycode == XKeysymToKeycode(keyrouter.disp, XStringToKeysym(KEY_VOICEWAKEUP)))
		{
			SLOG(LOG_DEBUG, "HIGEAR", "[36mVoice Wakeup Key Pressed![0m\n");
		}
...
_e_keyrouter_cb_client_message (void* data, int type, void* event)
{
	...
	else if(ev->format == 32)
	{
		...
		if( longpress_win && keycode && longpress_timeout )
		{
			...
			{
				msgsym = XKeycodeToKeysym(keyrouter.disp, (KeyCode)keycode, col);
				for(i=MIN_KEYCODE; i<MAX_HARDKEYS; i++)
				{
					if(XKeycodeToKeysym(keyrouter.disp, i, col) == msgsym)
					{
...
static void _e_keyrouter_do_bound_key_action(XEvent *xev)
{
	...
	{
		if (ev->keyname) free(ev->keyname);
		if (ev->key) free(ev->key);
		free(ev);
...
static void PrintKeyDeliveryList()
{
	...
	for( index=0 ; index < MAX_HARDKEYS ; index++ )
	{
		...
		if( keyrouter.HardKeys[index].keycode == 0 )//empty
			continue;
		ks = XKeycodeToKeysym(keyrouter.disp, index, 0);
...
static E_Border *_e_keyrouter_find_border_by_window(Window win)
{
...
static int IsWindowTopVisibleWithoutInputFocus(keylist_node* top_ptr, Window focus, Window * dest_window)
{
	...
	for (i = (int)num_children - 1; i >= 0; i--)
	{
		...
		bd = _e_keyrouter_find_border_by_window(child_list[i]);
...
static void DeliverDeviceKeyEvents(XEvent *xev)
{
	...
	if (xev->type == KeyPress)
	{
		switch( grab_mode )
		{
			...
			case TOP_POSITION_GRAB:
				...
				if( keyrouter.resTopVisibleCheck == 1 && dest_window != NULL)
	...
	shared_delivery:
				...
				for( ptr=keyrouter.HardKeys[index].shared_ptr ; (NULL != ptr) ; ptr=ptr->next )
				{
					...
					/*
					  * Below 2 lines will be removed.
					  * Send e msg to svoice-tizen
					  */
					if ((xev->xkey.keycode == XKeysymToKeycode(keyrouter.disp, XStringToKeysym(KEY_VOICEWAKEUP))) && (xev->type == KeyPress))
					{
						SECURE_SLOGD("Hi Gear, e_msg_send() called");
						e_msg_send("e.svoice.keyevent", "launch", 1, NULL, NULL, NULL, NULL);
					}
				}
				break;
