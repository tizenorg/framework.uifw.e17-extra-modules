        ...
        switch (angle)
          {
           ...
           case 180:
             move_y = quickpanel_move_y - m->quickpanel->border->h;
             opacity = MAX_OPACITY * (m->quickpanel->border->h - move_y) / m->quickpanel->border->h;
             break;
           case 270:
             move_x = quickpanel_move_x - m->quickpanel->border->w;
             opacity = MAX_OPACITY * (m->quickpanel->border->w - move_x) / m->quickpanel->border->w;
             break;
           case   0:
           default :
             move_y = quickpanel_move_y + m->quickpanel->border->h;
        ...
        switch (angle)
          {
           ...
           case 180:
             ...
             opacity = MAX_OPACITY * (m->app_tray->border->h - move_y) / m->app_tray->border->h;
             break;
           case 270:
             move_x = app_tray_move_x - m->app_tray->border->w;
             opacity = MAX_OPACITY * (m->app_tray->border->w - move_x) / m->app_tray->border->w;
             break;
           case   0:
           default :
             move_y = app_tray_move_y  + m->app_tray->border->h;
...
e_mod_move_show_apptray_helper(E_Move *m)
{
   ...
   ecore_event_handler_add(ECORE_EVENT_MOUSE_BUTTON_UP, apptray_helper->event.up, apptray_helper);
   ecore_event_handler_add(ECORE_EVENT_MOUSE_BUTTON_DOWN, apptray_helper->event.down, apptray_helper);
   ecore_event_handler_add(ECORE_EVENT_MOUSE_MOVE, apptray_helper->event.move, apptray_helper);
...
_e_mod_move_prop_indicator_state_get(Ecore_X_Window win)
{
   ...
   ret = ecore_x_window_prop_card32_get
           (win, ECORE_X_ATOM_E_ILLUME_INDICATOR_STATE, &val, 1);
   if (ret < 0) return -1;
   return val;
}
...
_e_mod_move_tp_indi_touch_win_show(E_Move *m)
{
   ...
   ecore_event_handler_add(ECORE_EVENT_MOUSE_BUTTON_UP, tp_indi->touch.ev_up, tp_indi);
   indi = _e_mod_move_bd_find_by_indicator(m);
