                                          ...
                                          FILE   *fs)
{
   ...
   fprintf(fs, "\n\nB-------------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
   fprintf(fs, "   <MOVE MODULE>  VISIBILITY / INDICATOR_STATE / INDICATOR_TYPE / FULLSCREEN INDICATOR_SHOW_STATE Info\n");
   fprintf(fs, "------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
   fprintf(fs, "  NO   BORDER  CLIENT_WIN | V | VISIBILITY_STATE | INDICATOR_STATE | INDICATOR_TYPE | FULLSCREEN INDICATOR_SHOW_STATE | ANGLE | PID  | WNAME / WCLASS\n");
   fprintf(fs, "------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
   ...
   EINA_INLIST_REVERSE_FOREACH(m->borders, mb)
     {
        ...
        switch (mb->fullscreen_indicator_show_state)
          {
           case E_MOVE_FULLSCREEN_INDICATOR_SHOW_STATE_NONE:
              snprintf(buf3, sizeof(buf3), "%s", "NONE");
              break;
           case E_MOVE_FULLSCREEN_INDICATOR_SHOW_STATE_ON:
              snprintf(buf3, sizeof(buf3), "%s", "STATE_ON");
              break;
           case E_MOVE_FULLSCREEN_INDICATOR_SHOW_STATE_OFF:
              snprintf(buf3, sizeof(buf3), "%s", "STATE_OFF");
              break;
           default:
              break;
          }
        ...
        fprintf(fs,
                " %3d 0x%07x %9s | %2s | %16.16s | %15.15s | %14.14s | %31.31s | %5d | %4d | %s / %s\n",
                i,
                bid,
                buf,
                mb->visible ? "v" : " ",
