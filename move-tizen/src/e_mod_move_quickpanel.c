...
static Eina_Bool          _e_mod_move_quickpanel_above_borders_list_init(E_Move_Border *qp_mb);
static Eina_Bool          _e_mod_move_quickpanel_above_borders_list_deinit(E_Move_Border *qp_mb);
static Eina_Bool          _e_mod_move_quickpanel_above_borders_list_search(E_Move_Border *qp_mb, E_Move_Border *mb);
static Eina_Bool          _e_mod_move_quickpanel_above_border_add(E_Move_Border *qp_mb, E_Move_Border *mb);
static Eina_Bool          _e_mod_move_quickpanel_above_border_del(E_Move_Border *qp_mb, E_Move_Border *mb);
static Eina_Bool          _e_mod_move_quickpanel_above_border_check(E_Move_Border *qp_mb, E_Move_Border *mb);
static Eina_Bool          _e_mod_move_quickpanel_above_borders_stack_align(E_Move_Border *qp_mb);
                                     ...
                                     void *event_info)
{
   ...
   cancel_state = e_mod_move_flick_scroll_cancel_state_get(mb, E_MOVE_FLICK_UP);
...
_e_mod_move_quickpanel_handle_objs_add(E_Move_Border *mb)
{
   ...
   m = mb->m;
   qp_data->handle_objs = e_mod_move_evas_objs_add(m, mb->bd, EINA_TRUE);
...
_e_mod_move_quickpanel_animation_change_with_angle(E_Move_Border *mb)
{
   ...
   if (!e_mod_move_util_win_angle_get(bd->client.win,
                                      &curr_angle,
                                      &prev_angle))
     return EINA_FALSE;
...
static Eina_Bool
_e_mod_move_quickpanel_above_borders_list_init(E_Move_Border *qp_mb)
{
   E_Move *m = NULL;
   E_Move_Quickpanel_Data *qp_data = NULL;
   E_Move_Border *find_mb = NULL;
   ...
   qp_data = (E_Move_Quickpanel_Data *)qp_mb->data;
   if (!qp_data)
      qp_data = (E_Move_Quickpanel_Data *)e_mod_move_quickpanel_internal_data_add(qp_mb);
   E_CHECK_RETURN(qp_data, EINA_FALSE);

   if (qp_data->stack_above_borders.list)
      _e_mod_move_quickpanel_above_borders_list_deinit(qp_mb);

   EINA_INLIST_REVERSE_FOREACH(m->borders, find_mb)
     {
        if (TYPE_QUICKPANEL_CHECK(find_mb)) break;
        if (find_mb->visible
            && REGION_INTERSECTS_WITH_ZONE(find_mb, qp_mb->bd->zone))
          {
             qp_data->stack_above_borders.list = eina_list_append(qp_data->stack_above_borders.list, find_mb);
             if (!(find_mb->objs))
               {
   ...
   qp_data->stack_above_borders.use = EINA_TRUE;

   return EINA_TRUE;
}

static Eina_Bool
_e_mod_move_quickpanel_above_borders_list_deinit(E_Move_Border *qp_mb)
{
   E_Move_Quickpanel_Data *qp_data = NULL;
   E_Move_Border *find_mb = NULL;
   Eina_List *l = NULL;
   ...
   if (qp_data->stack_above_borders.list)
     {
        EINA_LIST_REVERSE_FOREACH(qp_data->stack_above_borders.list, l, find_mb)
          {
             e_mod_move_bd_move_objs_del(find_mb, find_mb->objs);
             find_mb->objs = NULL;
             qp_data->stack_above_borders.list = eina_list_remove(qp_data->stack_above_borders.list, find_mb);
          }
     }

   qp_data->stack_above_borders.list = NULL;
   qp_data->stack_above_borders.use = EINA_FALSE;
...
static Eina_Bool
_e_mod_move_quickpanel_above_borders_list_search(E_Move_Border *qp_mb,
                                                 E_Move_Border *mb)
{
   E_Move_Quickpanel_Data *qp_data = NULL;
   E_Move_Border *find_mb = NULL;
   ...
   E_CHECK_RETURN(mb, EINA_FALSE);
   E_CHECK_RETURN(TYPE_QUICKPANEL_CHECK(qp_mb), EINA_FALSE);
   if (TYPE_QUICKPANEL_CHECK(mb)) return EINA_FALSE;

   qp_data = (E_Move_Quickpanel_Data *)qp_mb->data;
   E_CHECK_RETURN(qp_data, EINA_FALSE);

   if ((qp_data->stack_above_borders.use) && (qp_data->stack_above_borders.list))
     {
        find_mb = (E_Move_Border *)eina_list_data_find(qp_data->stack_above_borders.list, mb);
...
static Eina_Bool
_e_mod_move_quickpanel_above_border_add(E_Move_Border *qp_mb,
                                        E_Move_Border *mb)
{
   E_Move_Quickpanel_Data *qp_data = NULL;
   Eina_Bool ret = EINA_FALSE;
   ...
   if (qp_data->stack_above_borders.use)
     {
        qp_data->stack_above_borders.list = eina_list_append(qp_data->stack_above_borders.list, mb);
        if (!(mb->objs))
          {
...
static Eina_Bool
_e_mod_move_quickpanel_above_border_del(E_Move_Border *qp_mb,
                                        E_Move_Border *mb)
{
   E_Move_Quickpanel_Data *qp_data = NULL;
   Eina_Bool ret = EINA_FALSE;
   ...
   if (qp_data->stack_above_borders.list)
     {
        ...
        mb->objs = NULL;
        qp_data->stack_above_borders.list = eina_list_remove(qp_data->stack_above_borders.list, mb);
        if (eina_list_count(qp_data->stack_above_borders.list) == 0)
           qp_data->stack_above_borders.list = NULL;
...
static Eina_Bool
_e_mod_move_quickpanel_above_border_check(E_Move_Border *qp_mb,
                                          E_Move_Border *mb)
{
   E_Move *m = NULL;
   E_Move_Border *find_mb = NULL;
...
static Eina_Bool
_e_mod_move_quickpanel_above_borders_stack_align(E_Move_Border *qp_mb)
{
   E_Move *m = NULL;
   E_Move_Border *find_mb = NULL;
   E_Move_Border *qp_above_mb = NULL;
   E_Move_Border *stack_base_mb = NULL;
   E_Move_Quickpanel_Data *qp_data = NULL;
   Eina_List *l = NULL;
   Eina_Bool mirror = EINA_TRUE;
   ...
   if (!(qp_data->stack_above_borders.use)
       || !(qp_data->stack_above_borders.list))
      return EINA_FALSE;
   ...
   EINA_INLIST_REVERSE_FOREACH(m->borders, find_mb)
     {
        ...
        EINA_LIST_REVERSE_FOREACH(qp_data->stack_above_borders.list, l, qp_above_mb)
          {
             if (find_mb == qp_above_mb)
               {
                  qp_data->stack_above_borders.list = eina_list_remove(qp_data->stack_above_borders.list, qp_above_mb);
                  qp_data->stack_above_borders.list = eina_list_prepend(qp_data->stack_above_borders.list, qp_above_mb);

                  if (!qp_above_mb->objs)
                    {
                       qp_above_mb->objs = e_mod_move_bd_move_objs_add(qp_above_mb, mirror);
                       e_mod_move_bd_move_objs_move(qp_above_mb, qp_above_mb->x, qp_above_mb->y);
                       e_mod_move_bd_move_objs_resize(qp_above_mb, qp_above_mb->w, qp_above_mb->h);
                       e_mod_move_bd_move_objs_show(qp_above_mb);
                    }

                  if (!stack_base_mb)
                    {
                       e_mod_move_bd_move_objs_raise(qp_above_mb);
                       stack_base_mb = qp_above_mb;
                    }
                  else
                    {
                       e_mod_move_bd_move_objs_stack_below(qp_above_mb, stack_base_mb);
                       stack_base_mb = find_mb;
                    }
   ...
   if (!(mb->objs))
     {
        ...
        _e_mod_move_quickpanel_above_borders_list_init(mb);
     }
   return EINA_TRUE;
}
...
e_mod_move_quickpanel_objs_del(E_Move_Border *mb)
{
   ...
   // deinit quickpanel above boders list (delete above border's obj and free list)
   _e_mod_move_quickpanel_above_borders_list_deinit(mb);
...
e_mod_move_quickpanel_dim_hide(E_Move_Border *mb)
{
   ...
   L(LT_EVENT_OBJ,
     "[MOVE] ev:%15.15s w:0x%08x %s()\n",
     "EVAS_OBJ", ((mb->bd) ? mb->bd->win : (unsigned int)NULL), __func__);
   ...
   if (m->indicator_widget)
     {
        ...
        if (m->indicator_widget->target)
           target = m->indicator_widget->target;
        else
           target = m->indicator_widget->win;

        ecore_x_client_message32_send
                (target, ATOM_MV_QUICKPANEL_STATE,
                ECORE_X_EVENT_MASK_WINDOW_CONFIGURE,
                d[0], d[1], d[2], d[3], d[4]);
     }
...
EINTERN void e_mod_move_quickpanel_above_border_process(E_Move_Border *mb,
                                                        Eina_Bool      size,
                                                        Eina_Bool      pos,
                                                        Eina_Bool      stack,
                                                        Eina_Bool      visible)
{
   ...
   if (TYPE_QUICKPANEL_CHECK(mb)) return;
   ...
   if (!(qp_data->stack_above_borders.use)) return;

   is_above_border = _e_mod_move_quickpanel_above_border_check(qp_mb, mb);
   is_contain_above_border_list = _e_mod_move_quickpanel_above_borders_list_search(qp_mb, mb);

   if (!(is_above_border) && is_contain_above_border_list)
     {
        _e_mod_move_quickpanel_above_border_del(qp_mb, mb);
        return;
     }

   if ( is_above_border && !(is_contain_above_border_list))
     {
        _e_mod_move_quickpanel_above_border_add(qp_mb, mb);
        _e_mod_move_quickpanel_above_borders_stack_align(qp_mb);
     }
