...
static Eina_Bool      _e_mod_move_indicator_widget_click_check_and_mouse_up_send(E_Move_Indicator_Widget *indi_widget);
                                             ...
                                             void *event_info)
{
   ...
   SECURE_SLOGI("ev:%15.15s w:0x%08x INDI_WIDGET_MOTION_START (%4d,%4d)\n",
     "EVAS_OBJ", mb->bd->win,
   ...
   if ((qp_mb) &&
     ...
     {
        SECURE_SLOGE("ev:%15.15s w:0x%08x INDI_WIDGET_MOTION_START %s\n",
          "EVAS_OBJ", mb->bd->win,
...
error_cleanup:
   ...
   SECURE_SLOGE("ev:%15.15s w:0x%08x INDI_WIDGET_MOTION_START %s\n",
     "EVAS_OBJ",mb->bd->win,"e_mod_move_flick_data_new failed");
                                           ...
                                           void *event_info)
{
   ...
   SECURE_SLOGI("ev:%15.15s w:0x%08x ,angle:%d, (%d,%d)  %s()\n",
        ...
        for (index = num-1; index >= 0; index--)
          {
              ...
              if (!E_INSIDE(x, y, win_x, win_y,win_w, win_h))
               ...
               if (att.input_only && att.viewable)
                 {
                     *win = all_wins[index];
                     if (pointed_win) *pointed_win = all_wins[index];
                     found = EINA_FALSE;
                     ret = EINA_TRUE;
                     break;
                 }
...
#ifdef _DESK_LAYOUT_
   /* TODO: check E_Desk_Layout */
   if ((zone->id == 0)              // change zone->id comparing to bd's profile property (mobile)
...
static Eina_Bool
_e_mod_move_indicator_widget_click_check_and_mouse_up_send(E_Move_Indicator_Widget *indi_widget)
{
   E_CHECK_RETURN(indi_widget, EINA_FALSE);
   ...
   EINA_INLIST_REVERSE_FOREACH(m->borders, find_mb)
     {
        ...
        // if split mode is enabled then return.
        if (TYPE_SPLIT_LAUNCHER_CHECK(find_mb))
          continue;

        if ((find_mb->visible) &&
            (find_mb->bd->client.e.state.ly.curr_ly) &&
            REGION_INTERSECTS_WITH_ZONE(find_mb, zone))
          break;
        ...
        if ((find_mb->visible) && (zone->id == 0)) // check visible, change zone->id comparing to bd's profile property (mobile)
          {
             if (REGION_EQUAL_TO_ZONE(find_mb, zone))
               {
                  if (find_mb->fullscreen_indicator_show_state == E_MOVE_FULLSCREEN_INDICATOR_SHOW_STATE_OFF)
                    {
                       break;
                    }
        ...
        if ((indi_widget = e_mod_move_indicator_widget_get()))
          {
             ...
             if ((indi_widget->win == target_win))
             ...
             else
               {
                  ...
                  _e_mod_move_indicator_widget_click_check_and_mouse_up_send(indi_widget);
...
e_mod_move_indicator_widget_add(Ecore_X_Window win)
{
   ...
   if (e_mod_move_util_win_angle_get(win, &curr, &prev))
     {
        if (curr != mb->angle) mb->angle = curr;
     }
   else
     mb->angle = 0;
...
EINTERN Eina_Bool
e_mod_move_indicator_widget_fullscreen_indicator_show_state_change(Ecore_X_Window win, E_Move_Fullscreen_Indicator_Show_State state)
{
   E_Move_Indicator_Widget *indi_widget = NULL;
   E_Move_Border           *mb = NULL;
   ...
   if ((indi_widget = e_mod_move_indicator_widget_get()))
     {
        if ((indi_widget->win == win) &&
            (state == E_MOVE_FULLSCREEN_INDICATOR_SHOW_STATE_OFF))
          {
             // indicator fullscreen state is OFF -> delete current indicator widget
             e_mod_move_indicator_widget_del(indi_widget);
             e_mod_move_indicator_widget_set(NULL);
          }
     }
