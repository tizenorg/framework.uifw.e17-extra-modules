...
static Eina_Bool      _e_mod_move_bd_layout_set_begin(void *data __UNUSED__, int type __UNUSED__, void *event);
static Eina_Bool      _e_mod_move_bd_layout_set_cancel(void *data __UNUSED__, int type __UNUSED__, void *event);
static Eina_Bool      _e_mod_move_bd_layout_set_end(void *data __UNUSED__, int type __UNUSED__, void *event);
static Eina_Bool      _e_mod_move_bd_rotation_change_end(void *data __UNUSED__, int type __UNUSED__, void *event);
...
static void           _e_mod_move_bd_rotation_change_end_intern(E_Move_Border *mb, int angle);
...
static void           _e_mod_move_bd_adjust_stack_list(E_Border *bd);
                     ...
                     void *event __UNUSED__)
{
   ...
   else if (a == ECORE_X_ATOM_E_ILLUME_ROTATE_WINDOW_ANGLE)
                    ...
                    void      *event)
{
   ...
   else if (t == ECORE_X_ATOM_E_ILLUME_QUICKPANEL_STATE)
     {
        _e_mod_move_msg_qp_state(ev);
     }
...
static Eina_Bool
_e_mod_move_bd_layout_set_begin(void *data __UNUSED__,
                                int type   __UNUSED__,
                                void      *event)
{
   E_Event_Border_Desk_Layout_Set_Begin *ev = event;
   E_Move_Border                        *mb = NULL;
...
static Eina_Bool
_e_mod_move_bd_layout_set_cancel(void *data __UNUSED__,
                                 int type   __UNUSED__,
                                 void      *event)
{
   E_Event_Border_Desk_Layout_Set_Cancel *ev = event;
   E_Move_Border                         *mb = NULL;
...
static Eina_Bool
_e_mod_move_bd_layout_set_end(void *data __UNUSED__,
                              int type   __UNUSED__,
                              void      *event)
{
   E_Event_Border_Desk_Layout_Set_End *ev = event;
   E_Move_Border                      *mb = NULL;
...
static Eina_Bool
_e_mod_move_bd_rotation_change_end(void *data __UNUSED__,
                                   int type   __UNUSED__,
                                   void      *event)
{
   ...
   E_CHECK_RETURN(ev, ECORE_CALLBACK_PASS_ON);
   E_CHECK_RETURN(ev->border, ECORE_CALLBACK_PASS_ON);
   if (e_object_is_del(E_OBJECT(ev->border))) return ECORE_CALLBACK_PASS_ON;

   L(LT_EVENT_BD,
     "[MOVE] ev:%15.15s w:0x%08x c:0x%08x angle:%d\n", "BD_ROTATION_CHANGE_END",
     ev->border->win, ev->border->client.win, e_border_rotation_curr_angle_get(ev->border));

   mb = _e_mod_move_border_find(ev->border->win);
   E_CHECK_RETURN(mb, ECORE_CALLBACK_PASS_ON);

   angle = e_border_rotation_curr_angle_get(ev->border);

   _e_mod_move_bd_rotation_change_end_intern(mb, angle);

   return ECORE_CALLBACK_PASS_ON;
}

static Eina_Bool
_e_mod_move_zone_rot_end(void *data __UNUSED__,
                         int type   __UNUSED__,
                         void      *event)
{
   ...
   e_mod_move_split_mode_indicator_widget_zone_rot_change();

   return ECORE_CALLBACK_PASS_ON;
}

static Eina_Bool
_e_mod_move_comp_source_visibility(void *data __UNUSED__,
                                   int type   __UNUSED__,
                                   void      *event)
{
   ...
   bd = e_border_find_all_by_client_window(ev->win);
   E_CHECK_RETURN(bd, ECORE_CALLBACK_PASS_ON);

   mb = _e_mod_move_border_find(bd->win);
   E_CHECK_RETURN(mb, ECORE_CALLBACK_PASS_ON);

   e_mod_move_quickpanel_above_border_process(mb, EINA_FALSE, EINA_FALSE, EINA_FALSE, ev->visible);
   e_mod_move_split_launcher_above_border_process(mb, EINA_FALSE, EINA_FALSE, EINA_FALSE, ev->visible);
   return ECORE_CALLBACK_PASS_ON;
}
                        ...
                        void      *data2)
{
   ...
   _e_mod_move_bd_adjust_stack_list(bd);
}

static void
_e_mod_move_bd_adjust_stack_list(E_Border *bd)
{
   ...
   E_CHECK(bd);
   /* need to adjust stack list when bd stack event is faster than new hook */
   bl = e_container_border_list_last(bd->zone->container);
   E_CHECK(bl);

   while ((find_bd = e_container_border_list_prev(bl)))
     {
        find_mb = _e_mod_move_border_find(find_bd->win);
        if (!find_mb) continue;
        _e_mod_move_bd_stack_intern(find_mb, NULL, E_MOVE_STACKING_LOWER);
     }
                          ...
                          void       *msgdata)
{
   ...
   else if (!strncmp(name, "e.move", sizeof("e.move")))
     {
        if (!strncmp(info, "split_mode_indicator.event_forward_off", sizeof("split_mode_indicator.event_forward_off")))
          {
             L(LT_EVENT_X, "[MOVE] ev:E_MSG [%s:%s]\n", name, info);
             e_mod_move_split_mode_indicator_widget_event_forward_off_set(EINA_TRUE);
          }
        else if (!strncmp(info, "split_mode_indicator.event_forward_on", sizeof("split_mode_indicator.event_forward_on")))
          {
             L(LT_EVENT_X, "[MOVE] ev:E_MSG [%s:%s]\n", name, info);
             e_mod_move_split_mode_indicator_widget_event_forward_off_set(EINA_FALSE);
          }
     }
}
...
_e_mod_move_bd_add_intern(E_Move_Border *mb)
{
   ...
   if (e_mod_move_util_win_angle_get(win, &angles[0], &angles[1]))
     angles[0] %= 360;
   else
     angles[0] = 0;
   mb->angle = angles[0];
...
_e_mod_move_bd_del_intern(E_Move_Border *mb)
{
   ...
   // For Quickpanel and Split launcher Above Border processing.
   e_mod_move_quickpanel_above_border_process(mb, EINA_FALSE, EINA_FALSE, EINA_FALSE, EINA_FALSE);
   e_mod_move_split_launcher_above_border_process(mb, EINA_FALSE, EINA_FALSE, EINA_FALSE, EINA_FALSE);
...
_e_mod_move_bd_show_intern(E_Move_Border *mb)
{
   ...
   // For Quickpanel and Split launcher Above Border processing.
   e_mod_move_quickpanel_above_border_process(mb, EINA_FALSE, EINA_FALSE, EINA_FALSE, EINA_TRUE);
   e_mod_move_split_launcher_above_border_process(mb, EINA_FALSE, EINA_FALSE, EINA_FALSE, EINA_TRUE);
}
...
_e_mod_move_bd_hide_intern(E_Move_Border *mb)
{
   ...
   // For Quickpanel Above Border processing.
   e_mod_move_quickpanel_above_border_process(mb, EINA_FALSE, EINA_FALSE, EINA_FALSE, EINA_TRUE);
   e_mod_move_split_launcher_above_border_process(mb, EINA_FALSE, EINA_FALSE, EINA_FALSE, EINA_TRUE);
}
   ...
   if (!((w == mb->w) && (h == mb->h)))
     {
        ...
        // For Quickpanel and Split launcher Above Border processing.
        e_mod_move_quickpanel_above_border_process(mb, EINA_TRUE, EINA_FALSE, EINA_FALSE, EINA_FALSE);
        e_mod_move_split_launcher_above_border_process(mb, EINA_TRUE, EINA_FALSE, EINA_FALSE, EINA_FALSE);
     }
   ...
   if (!((x == mb->x) && (y == mb->y)))
     {
        ...
        // For Quickpanel and Split launcher Above Border processing.
        e_mod_move_quickpanel_above_border_process(mb, EINA_FALSE, EINA_TRUE, EINA_FALSE, EINA_FALSE);
        e_mod_move_split_launcher_above_border_process(mb, EINA_FALSE, EINA_TRUE, EINA_FALSE, EINA_FALSE);
     }
}
                            ...
                            E_Move_Stacking type)
{
   ...
   // For Quickpanel and Split launcher Above Border processing.
   e_mod_move_quickpanel_above_border_process(mb1, EINA_FALSE, EINA_FALSE, EINA_TRUE, EINA_FALSE);
   e_mod_move_split_launcher_above_border_process(mb1, EINA_FALSE, EINA_FALSE, EINA_TRUE, EINA_FALSE);
}

static void
_e_mod_move_bd_rotation_change_end_intern(E_Move_Border *mb,
                                          int            angle)
{
   E_Move        *m = NULL;
   ...
   if (m->elm_indicator_mode)
     {
        e_mod_move_indicator_widget_angle_change(win);
        e_mod_move_indicator_widget_angle_change_post_job();
     }

   //for split indicator mode
   e_mod_move_split_mode_indicator_widget_angle_change(win);
...
_e_mod_move_bd_internal_data_add(E_Move_Border *mb)
{
   ...
   else if (TYPE_SPLIT_LAUNCHER_CHECK(mb))
      ret = e_mod_move_split_launcher_internal_data_add(mb);
   return ret;
}
...
_e_mod_move_bd_internal_data_del(E_Move_Border *mb)
{
   ...
   if (TYPE_APPTRAY_CHECK(mb))
     ...
     ret = e_mod_move_mini_apptray_internal_data_del(mb);
      ...
      ret = e_mod_move_split_launcher_internal_data_del(mb);
   return ret;
}
   ...
   if (_e_mod_move_prop_active_window_get(&active_win))
     {
        ...
        E_CHECK_RETURN(active_mb->bd, EINA_FALSE);
        if (e_object_is_del(E_OBJECT(active_mb->bd)))
           return EINA_FALSE;

        active_pid = active_mb->bd->client.netwm.pid;
   ...
   if (_e_mod_move_prop_indicator_state_get(ev->win, &indicator_state))
     {
        ...
        // If indicator State go off, then apply split_indicator_mode
        e_mod_move_split_mode_indicator_widget_state_change(ev->win, indicator_state);
     }
...
/* WORKAROUND
 * since camera application rotate its window itself without help of window manager.
 * so, we should keep watch the property of camera window's rotation angle..
 * and then if camera window is rotated, we should change the angle of indicator widget.
 * FIXME: update below 2 function so that more efficiently.
 */
static Eina_Bool
_e_mod_move_border_is_camera(E_Border *bd)
{
...
_e_mod_move_prop_rotate_window_angle(Ecore_X_Event_Window_Property *ev)
{
   ...
   E_CHECK_RETURN(_e_mod_move_border_is_camera(bd), EINA_FALSE);
...
_e_mod_move_prop_fullscreen_indicator_show_state(Ecore_X_Event_Window_Property *ev)
{
   ...
   e_mod_move_indicator_widget_fullscreen_indicator_show_state_change(ev->win, mb->fullscreen_indicator_show_state);
   ...
   if (open)
     {
        ...
        if (mb == sl_mb && !REGION_INSIDE_ZONE(mb, zone))
          state = EINA_FALSE;
     }
   else
     {
   ...
   if ((open) && (!state) && (comp_obj_visible))  // Open Case
     {
        ...
        else if (mb == sl_mb) // Split Launcher case
          {
             e_mod_move_split_launcher_show(mb);
          }
     }
   else if ((!open) && (state)) // Close Case
     {
        if (mb == qp_mb) // Quickpanel case
        ...
        else if (mb == mini_at_mb) // Mini_Apptray Case
          {
             ...
             if (animation)
               ...
               {
                  e_mod_move_mini_apptray_e_border_move(mini_at_mb, x, y);
                  e_mod_move_mini_apptray_dim_hide(mini_at_mb);
               }
        ...
        if (mb == sl_mb) // Split Launcher case
          {
             e_mod_move_split_launcher_hide(mb);
          }
     }
   else
     {
...
e_mod_move_init(void)
{
   ...
   handlers = eina_list_append(handlers, ecore_event_handler_add(E_EVENT_BORDER_DESK_LAYOUT_SET_BEGIN,   _e_mod_move_bd_layout_set_begin,    NULL));
   handlers = eina_list_append(handlers, ecore_event_handler_add(E_EVENT_BORDER_DESK_LAYOUT_SET_CANCEL,  _e_mod_move_bd_layout_set_cancel,   NULL));
   handlers = eina_list_append(handlers, ecore_event_handler_add(E_EVENT_BORDER_DESK_LAYOUT_SET_END,     _e_mod_move_bd_layout_set_end,      NULL));
   handlers = eina_list_append(handlers, ecore_event_handler_add(E_EVENT_BORDER_ROTATION_CHANGE_END,     _e_mod_move_bd_rotation_change_end, NULL));
   handlers = eina_list_append(handlers, ecore_event_handler_add(E_EVENT_ZONE_ROTATION_CHANGE_END,       _e_mod_move_zone_rot_end,           NULL));
   handlers = eina_list_append(handlers, ecore_event_handler_add(E_EVENT_COMP_SOURCE_VISIBILITY,         _e_mod_move_comp_source_visibility, NULL));

   e_border_hooks = eina_list_append(e_border_hooks, e_border_hook_add(E_BORDER_HOOK_POST_NEW_BORDER,    _e_mod_move_bd_new_hook,            NULL));
